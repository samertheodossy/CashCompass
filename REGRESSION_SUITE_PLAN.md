...
| Bills Due → Pay | Bills recurrence and Cash Flow linkage automated | `SUITE-BILLS-REGRESSION`; `SUITE-BILLS-PAY-E2E` runtime PASS at `@120`: explicit disposable-ss Quick Add → Cash Flow → Activity → handled-marker dedupe, 3/3, Provisioning/Drift/Trash PASS. Dashboard UX additionally requires one visible **Bill paid** monetary Activity row while retaining the hidden occurrence marker, plus focused `$0.00` drawer formatting. Local read-model and focus regressions passed and isolated Central `@291` visually confirmed one `$234.00` **Bill paid** row plus a focused, fully selected `$550.00` amount with no browser warnings/errors; the drawer was canceled without a workbook write. | Synthetic regression does not replace separately required natural cohort evidence. |
...
|---|---|---|---|
| Overview health prerequisite/freshness (`3l` / `REG-021`) | Score availability and label reconcile with the shared five-step Setup readiness contract, current-month planner history, and contradictory cash conditions | Dynamic Dashboard UX regression proves Setup-incomplete, stale, contradictory, and healthy-current states | Isolated `@197`: First-Run V5 11/11 PASS and Populated V4 14/14 PASS, including `health_prerequisite_truth`; rerun on the exact candidate |
| Unattended browser orchestration (`1f` / `REG-022`) | Fixed identity route guards, no caller-selected email/workbook target, and exact marker/owner/mapping verification remain authoritative | Each server-rendered unattended control page self-starts only after its existing route identity gate passes | `@201` First-Run 11/11 and Populated 14/14 passed in the dedicated test session with Restricted single-owner fixtures and verified Trash cleanup; source regression rejects AccountChooser popup orchestration |
| Bank/Debt controlled loading resilience (`1c` / `REG-023`) | Existing selected-record identity and request-token gates remain authoritative; no writer is invoked by the injected journey | Populated V5 requires controlled failure, disabled Save, bounded retry recovery, and newest-before-stale completion for both Bank and Debt | Isolated `@203` Populated V5 passed 16/16, including `bank_loading_resilience` and `debt_loading_resilience`, with zero errors and verified exact-fixture Trash cleanup |
| Bill Skip/Stop-tracking safety (`1d` / `REG-024`) | Skip fallback is active-bill-gated; existing Cash Flow writes remain target-resolved and blank-only; Stop keeps row+payee stale protection and soft deactivation | Dashboard UX locks confirmation, marker-only/monthly suppression, action-store invalidation, newest-response gating, and customer wording; Populated V6 requires ordered Skip → stale Stop → valid Stop plus exact-fixture lifecycle inspection | Isolated `@206` Populated V6 passed 17/17, including `bill_skip_stop_safety`, zero browser errors, Restricted one-owner sharing, and verified exact-fixture Trash cleanup |
| Retirement prerequisite/results truth (`1e` / `REG-025`) | Scenario rows resolve by exact labels so compact and legacy sheet layouts share one read/write path | Static UX coverage keeps result walls hidden during guidance; First-Run V6 requires `retirement_guidance_not_ready`; Populated V7 requires `retirement_ready_results` | Isolated `@211`: First-Run 12/12 and Populated 18/18 PASS, zero errors, Restricted one-owner fixtures, verified Trash cleanup |
| Quick Add cumulative language (`2d` / `REG-026`) | Production Quick Add keeps using the additive Cash Flow helper (`currentValue + addValue`) | Action copy and Help both state add-to-total and do-not-replace semantics | Run the Quick Add reliability browser round-trip on isolated Central after deployment; rerun on the exact candidate |
| Upcoming Dismiss consequences (`2e` / `REG-027`) | Dismiss remains a status-only soft removal with an Activity lifecycle event and no Cash Flow writer | Action surface, hint, success message, and Help state no payment, no Cash Flow change, and preserved Upcoming/Activity history | Confirm the Dismiss copy on isolated Central after deployment; rerun on the exact candidate |
| User-facing appended-row formatting (`REG-028`) | Upcoming Add writes the same 11 values and reasserts Due Date/currency formats; Bills, Debts, and Donations retain their existing values and financial formats | Source regression requires sibling `PASTE_FORMAT` plus row-height copy and canonical first-row fallbacks across Upcoming, Bills, Debts, and Donations; Upcoming’s helper is also prohibited from sheet-wide restyling or data writes | Add one item in each flow on harness-created disposable workbooks; compare appended format/height with a populated sibling and cover the first-row fallback, then verify values and existing rows are unchanged |
| Activity operation identity and exact target state (`5h` / `REG-029`) | Existing Activity columns remain unchanged; every new row receives a unique event ID; one Quick Add action shares one server-owned operation ID and scoped/versioned targets | Dashboard UX dynamically verifies envelope creation, unique IDs, legacy/audit-only classification, and changed/cross-workbook rejection; P1 evidence locks the exact marker-verified runner and server preview | Isolated `@214` Populated V8 passed 19/19, including `activity_operation_envelope`, zero browser errors, Restricted single-owner sharing, and verified Trash cleanup; rerun on the exact candidate |
| Quick Add customer language (`REG-032`) | Prior-month reads keep their existing read-only year/payee/month lookup semantics | Dynamic source regression executes missing-year, missing-payee, and missing-month branches and rejects internal worksheet terminology in returned customer copy | Confirm the new-payee Quick Add preview on isolated Central `@219`; retain the broad customer-language scan |
| Consecutive Quick Add receipt ordering (`REG-033`) | Existing compare-and-set inspection remains authoritative and workbook-scoped | Dynamic UI regression proves a late obsolete response cannot restore its receipt or warn against the newer cumulative total | Repeat `$100` then `$25` against the same disposable payee and confirm `$125` with no yellow warning |
| Correction-result reconciliation (`REG-034`) | The guarded correction scenario continues proving exact Cash Flow and optional credit-card restoration | Dynamic renderer regression requires reversed amount plus before/current values in the success panel | Correct both cash and credit-card Quick Adds on isolated Central and confirm the visible values match the verified server result |
| Quick Add sequence correction (`REG-035`) | Operation identity, immutable before/after targets, workbook/actor scope, and exact current-state verification remain authoritative | Pure Cash Flow/card chains and the registered guarded scenario correct middle and earlier entries, then add and independently preview a newer entry | Populated Dashboard `activity_operation_envelope` must reverse newest, middle, and earlier entries while preserving later values |
| Confirmed Quick Add mismatch (`REG-036`) | Compare-and-set verification remains fail-closed for a stable genuine mismatch | Dynamic receipt regression requires two matching mismatch observations before warning and proves MATCH resets the evidence | Repeat consecutive Quick Adds and confirm no transient yellow banner; separately retain the changed-value refusal journey |
| Quick Add completion latency (`REG-037`) | The money write, target snapshots, and Activity envelope remain synchronous | Source/performance regression prohibits presentation-only prior/history reads from the writer; client refreshes them quietly after success | Record perceived save completion and confirm prior-month/chart content fills without extending the authoritative save |
| Correction date and result presentation (`REG-038`) | Immutable target locator remains the source of entry date and Cash Flow month | Renderer/source regression rejects raw parsed dates and locks concise month/date plus soft-blue completion styling | Confirm the correction preview and result use a concise date/month and show reversed, before, and current values |
| Same-target receipt retirement after correction (`REG-039`) | The correction result remains the authority for the corrected Cash Flow target; unrelated receipts continue normal monitoring | Dynamic browser test retires a later same-target receipt, suppresses its in-flight response, preserves an unrelated receipt, and closes an active warning | Guarded Populated journey injects the real later-operation receipt from its three-entry sequence, performs middle correction, returns to Activity, and requires no yellow warning; every candidate replay also receives a live rendered-screen inspection for warning banners, stuck progress, raw worksheet terms, and browser errors |
| Same-page Quick Add reconciliation after correction (`REG-040`) | Returning to Quick Add without reloading must show the corrected monthly value and history while preserving the form | Dynamic UI regression preserves type/payee/date, refreshes the payee inventory, sequences preview responses, and requires a quiet authoritative preview | Guarded Populated journey injects a stale visible total/history after the real correction sequence, invokes the production reconciliation path, and requires the corrected value/chart plus unchanged selection without a browser reload |
...
| ✅ Form readiness/customer labels (`3m`) | Required-field validators remain fail-closed against direct/forged calls | Primary actions disable until minimum valid input; Bill payment-source labels are customer-facing while stored values remain unchanged | Isolated `@325`: First-Run 16/16 and `@324`: Populated 24/24, with Restricted fixtures and verified cleanup |
...
| ✅ Residual Help and customer language (`3h`, `3m`) | Stored enums and routes remain compatible | No “2026 block,” no duplicated Retirement prerequisite, no internal tokens on normal paths | First-Run/Populated language sweep passed on isolated `@325`/`@324` |
...
| ✅ Money-format consistency (`3n` / `REG-067`) | Stored numeric values and writers remain unchanged | Every normal-path monetary renderer uses signed `$x.xx`; percentages, counts, ages, and inputs are excluded | Static formatter execution plus isolated `@331` Populated V12 run `FR-f7719f23-9248-41fa-9f5c-7327db963ce3` 26/26 PASS; clean console, Restricted sharing, verified cleanup |
...
| ✅ Retirement selected-first loading (`REG-066`) | Existing scenario math, Save persistence, and legacy full-reader compatibility remain unchanged | Overview's authoritative selected payload paints immediately; alternative comparisons background-load with generation and persisted-selection stale guards | Isolated `@328` Populated V11 run `FR-72b047d7-4f26-469a-9287-2bfda293023e` passed 25/25; selected/all comparisons measured 1 ms/5.648 s; Restricted sharing and cleanup verified |
...
| `REGRESSION-BILLS-WEEKDAY-AUTOPAY-GUARD` **✅ implemented; runtime pending** | Weekly AutoPay schedule authority | Due Day=1, Weekday=Sunday/Monday; plus missing Weekday | only the configured weekday occurrence writes; calendar-date Activity identity does not drift; missing/mismatched Weekday refuses AutoPay and leaves the occurrence visible. Local source gate passed and bounded user behavior passed on 2026-08-03; disposable scenario has not yet run. |
| `REGRESSION-BILLS-AUTOPAY-ROLLBACK` **✅ implemented; runtime pending** | AutoPay audit failure | past-due fixed monthly AutoPay with forced `appendActivityLog_` failure | exact prior Cash Flow value/format restored; no false marker; occurrence remains visible. Local source gate passed; disposable scenario has not yet run. |
...
| `REGRESSION-BILLS-31ST` **✅ implemented** | 31st-of-month | Due Day=31, Monthly, anchored Apr 15 and Jul 30 2026 | Mar→Mar 31; Apr→**Apr 30**; May→May 31; exact reported boundary generates Jun 30 / Jul 31 / Aug 31 and explicitly refuses a fabricated Jul 1 occurrence — `test_harness_scenarios_bills.js` |
| `REGRESSION-BILLS-LEAP-FEB29` **✅ implemented** | February high days | Due Day=29 and 30, Monthly, anchors Feb 10 2028 (leap) & Feb 10 2027 (non-leap) | leap February clamps high days to Feb 29; non-leap February clamps them to Feb 28; each logical February occurrence stays in February — `test_harness_scenarios_bills.js` |
...
9b. `REGRESSION-BILLS-31ST` + `REGRESSION-BILLS-LEAP-FEB29` — `[pure]` — **✅ implemented** (pin last-valid-day clamping for Monthly high Due Days, including the Jun-30 / Jul-31 boundary)
...
5. **31st-of-month & boundary expectations — resolved 2026-07-30.** Monthly,
   Yearly, and other one-occurrence-per-month schedules clamp Due Days 29/30/31
   to the last valid calendar day of a short month. Weekly/Biweekly retain their
   existing clamped anchor behavior.
