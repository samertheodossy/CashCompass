...
| Unattended browser orchestration (`1f`) | Fail closed unless the fixed non-admin disposable identity, exact candidate, enabled runner state, and harness-owned marker all reconcile; restore the runner flag to OFF in `finally` | One operator action shows queued/running/completed/paused-for-auth states and never asks for an email or workbook ID | Exact-run ownership, stale-evidence refusal, failure cleanup, exact-fixture Trash verification, and runner-flag restoration pass; expired auth produces one actionable sign-in pause |
