...
| `REGRESSION-BILLS-WEEKDAY-AUTOPAY-GUARD` **✅ implemented** | Weekly AutoPay schedule authority | Due Day=1, Weekday=Sunday/Monday; plus missing Weekday | only the configured weekday occurrence writes; calendar-date Activity identity does not drift; missing/mismatched Weekday refuses AutoPay and leaves the occurrence visible |
| `REGRESSION-BILLS-AUTOPAY-ROLLBACK` **✅ implemented** | AutoPay audit failure | past-due fixed monthly AutoPay with forced `appendActivityLog_` failure | exact prior Cash Flow value/format restored; no false marker; occurrence remains visible |
