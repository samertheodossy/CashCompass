...
| **Recent-session permanent regression packs** — Performance Planner and Bills Pay E2E runtime PASS at `@120`; First-Run UX E2E historical browser PASS 8/8 at `@128`, now expanded to a nine-assertion V2 customer-language contract that requires an isolated rerun; remaining `SUITE-POPULATED-DASHBOARD-E2E` and `SUITE-RECOVERY-LIVE`; aggregate in bounded chunks | P1–P3 by dependency | Central Safety suite + real browser/disposable-account seams | L–XL |
| Financial-integrity gate wiring into release | P1 | FI Phase 3 convergence | S |
| Deployment checklist | P2 | — | S |

> **Validator Phase 2 Workbook Health runtime gate completed 2026-07-21.** Provisioning, Workbook Drift, Schema Evolution V1, Formula, Conditional Formatting, Named Ranges, the formal schema registry, and aggregate Workbook Health are implemented and passed on isolated Central `@122`. First-Run UX E2E then passed its original 8/8 contract at `@128`; customer-language hardening expanded that contract to nine assertions and invalidated the older saved evidence until an isolated V2 rerun passes. The Validator remains read-only. Remaining P1 scope is that First-Run V2 rerun, Populated Dashboard E2E, Recovery Live, performance-budget ratification, Release Readiness console controls, and the final bounded verdict.
