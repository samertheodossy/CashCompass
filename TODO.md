...
2. **Exact-candidate browser evidence trust (`REG-015`; standalone fail-closed path runtime-proven on isolated `@178`, dedicated exact-owner proof pending).** Standalone Populated Dashboard PASS reports on isolated `@175`–`@177` inherited the parked `@141` `IN_PROGRESS` run and incorrectly saved release-eligible evidence. Generic/direct launches now carry no owner and are diagnostic-only even while an old run remains active. The final `@178` run passed 12/12 with zero captured errors, `releaseEligible: false`, no candidate or release run id, Restricted sharing, and verified Trash cleanup; two preceding attempts also failed closed and cleaned up but exposed an intermittent Debt timeout / Apps Script HTTP 0 reliability concern. Only a dedicated action in the active Release Readiness table supplies its exact run id; the admin launcher validates that id, campaign preparation captures it, and completion revalidates it. Diagnostic Performance cannot ratify a release budget. P1 evidence, production-path, syntax, and full local regressions pass.
...
These contained visibility fixes are covered by exact source-contract assertions in the existing `test:dashboard-ux` suite. Both default-width and 390px isolated `@177` Populated Dashboard journeys passed with verified Trash cleanup; no separate regression identifier is required. The advocate baseline is **8.1/10**. After the isolated `@178` full 12/12 PASS and standalone `REG-015` proof, the provisional score is **8.2/10**; the increase is limited because two preceding attempts exposed an intermittent Debt timeout / Apps Script HTTP 0 reliability concern.
