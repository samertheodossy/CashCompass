...
Houses and Income remain deferred under `3o`. Bills is source-complete in the
current candidate with counted inactive inventory and guarded Reactivate;
isolated runtime and bounded owner acceptance remain pending.
...
Houses and Income (`3o`; Bills runtime proof remains pending); separate
...
**The current at-a-glance priority stack now lives in its own document: `ROADMAP.md`** (created 2026-07-12, after the Validator-driven Golden/Canonical convergence milestone closed). `ROADMAP.md` is the authoritative **priority and sequence** view: P0 and P1 engineering are complete; P2 **House Financial Accuracy V1** and **Financial Integrity Phase 3** are complete; the `@318` Planner percentile gate, `@319` cross-flow measurement, and `@322` ordinary Save investigation are complete; P3 experience remains active; and the complete frozen-candidate verdict still returns before broad Beta. The roadmap carries the full House Financial Accuracy spec.
...
| Item | Priority | Dependencies | Effort |
|---|---|---|---|
| **Aggregate Release Readiness orchestration and console controls — ✅ COMPLETE**; `@318` Planner percentile evidence is ratified, while the complete final exact-candidate verdict remains open | P1 | Stable read models ✓ + required Validator/Harness coverage ✓ | — |
...
| **Recent-session permanent regression packs** — Performance Planner smoke and Bills Pay E2E PASS at `@120`; `@141` Workbook Health + 13/13 server checks + First-Run UX + Populated Dashboard + Recovery Live PASS; exact-owner `@318` Planner campaign PASS/ACCEPT and budget-ratified; production-path audit enforced | P1–P3 by dependency | Central Safety suite + real browser/disposable-account seams | L–XL |
| Financial-integrity gate wiring into release | P1 | FI Phase 3 convergence | S |
| Deployment checklist | P2 | — | S |

> **Validator Phase 2, scenario packs, and Release Readiness controls completed 2026-07-21; Planner percentile gate completed 2026-08-05.** The historical `@141` aggregate passed Workbook Health, 13/13 server checks, First-Run UX, Populated Dashboard, and Recovery Live. Exact-owner source `8aa4bf5` / isolated `@318` later completed all 20 Planner pairs with Restricted fixtures, verified cleanup, PASS/ACCEPT, and ratified 30/60 s budgets. The performance-only owner correctly archived overall `NOT_READY` because the other exact-candidate suites/server inventory were absent. The Validator remains read-only; the complete final-candidate verdict remains mandatory before broad Beta.
...
| Shared Lifecycle rollout — **Houses · Income Sources** plus Bills runtime proof | P3 | Debt, Bank Accounts, and Investments complete; Bills source-complete | M each |
...
4. **Validator / Test Harness / Release Readiness** — historical July foundation was runtime-proven through isolated `@141`; current correction: exact-owner Planner percentiles are ratified for `8aa4bf5` / `@318`, while the complete frozen-candidate verdict remains open.
...
- **Shared Lifecycle Framework rollout** — Debt is the reference implementation; Bank Accounts and Investments are complete; Bills is source-complete pending runtime proof; finish **Houses → Income Sources** (see `## Shared Lifecycle Framework` below).
...
**Rollout order (remaining implementation):** **Houses → Income Sources**. Debt is the shipped reference; Bank Accounts and Investments are complete. Bills now has counted inactive inventory and guarded Reactivate in the current source candidate; disposable runtime proof and bounded owner acceptance remain.
...
- **Validator / Test Harness / Release Readiness** — required automated evidence passes. *(Validator Phase 2, Test Harness Foundation V1, required non-performance scenario packs, bounded Release Readiness controls, `4d` cross-flow measurement, and the `4f` ordinary Save investigation are complete. Exact-owner Planner percentile evidence is ratified for `8aa4bf5` / `@318`; a conditional frozen-candidate replay and the complete final archived verdict remain mandatory before broad Beta. See `VALIDATOR_ARCHITECTURE.md`.)*
- **Recovery Validation** — ✅ full P0 matrix validated; read-only Orphan detection remains P1.
- **Runtime regression checklist** — core workflows + edge cases verified. *(Automated bounded and browser evidence covers the established inventory, the `@318` Planner gate, diagnostic `@319` cross-flow measurement, and `@322` ordinary Save follow-up; complete frozen-candidate evidence and product/human gates remain.)*
...
**Status:** implemented locally as a display/navigation-only pass and reviewed by the user on a populated dashboard. Three approved visual corrections are included; full local regressions pass. Ready for source-control checkpoint.
...
2. **Exact-candidate browser evidence trust (`REG-015` and `REG-019` runtime-closed).** Standalone Populated Dashboard PASS reports on isolated `@175`–`@177` inherited the parked `@141` `IN_PROGRESS` run and incorrectly saved release-eligible evidence. Generic/direct launches now carry no owner and are diagnostic-only even while an old run remains active; `@178` proved that fail-closed path. Isolated `@181` exact-owner run `RR-0e6941fb-6548-4c45-b5c3-6304ad0af686` then passed Workbook Health, 15/15 disposable server checks, and browser run `FR-13656973-a9c6-49ed-a54a-d6731daf01b6` with 12/12 assertions, exact candidate/run ownership, Restricted sharing, zero errors, and verified Trash cleanup. That replay exposed `REG-019`; the `@182` fix now makes **Refresh status** reconcile the saved evidence as `PASS / Verified`, persist it, and keep the run `IN_PROGRESS`. Dynamic coverage permanently checks that contract. `REG-018 / 1a` is complete: recovery remains bounded to verified pure reads and is proved by deterministic injected coverage plus the clean `@181` deployed integration replay; no unsupported forced-live transport claim is made. Diagnostic Performance cannot ratify a release budget.
   - **`1f` / `REG-022` unattended evidence accelerator — runtime complete on isolated `@201`.** The agent keeps a dedicated fixed non-admin test session and opens guarded self-starting routes directly, avoiding unreliable cross-account popup redirects. First-Run passed 11/11 and Populated passed 14/14 with Restricted single-owner fixtures and verified Trash cleanup. Google password and 2FA remain the only human authentication boundary when that dedicated session expires.
...
4. **✅ `3b` · P2 — Honest empty/guidance states — complete in `3e2db4f`
   ...
   `origin/main`.
5. **✅ `3m` · P2/P3 — Form readiness and residual customer language complete
   on isolated `@325`.** Primary actions remain disabled until minimum input is
   valid; server validators are preserved; Bill payment-source labels are
   customer-facing without changing stored tokens; obsolete and duplicated
   normal-path guidance is removed. Populated 24/24 and First-Run 16/16 passed
   with Restricted fixtures and verified cleanup.
...
7. **✅ `3i` · P3 — Medium composition complete on isolated `@325`.** The
   deliberate medium breakpoint, compact header, two-column Setup composition,
...
Work through these criteria **one at a time in this order**. Each row is a separate
reviewable wave: confirm the deductions, make the smallest coherent change, add
permanent regression coverage, deploy only to isolated Central, rerun the relevant
desktop / medium / 390px journey, and let the independent advocate rescore it.
A criterion reaches 10/10 only when no material issue or evidence gap remains in
its defined scope; code intent alone cannot raise the score.
...
4. **Validator / Test Harness / Release Readiness** — ✅ historical `@141` non-performance foundation complete; the `@318` Planner gate is ratified, `4d` cross-flow measurement passed on `@319`, and `4f` ordinary Save investigation closed on `@322` at 6.520/9.418 s p50/p95 with the strict 6 s near-miss documented. Conditional frozen-candidate replay and the complete final verdict remain required.
...
4. **Validator / Test Harness / Release Readiness** — ✅ historical `@141` non-performance foundation complete; under `ROADMAP.md`, the `@318` Planner gate is ratified, `4d` cross-flow measurement passed on `@319`, and `4f` ordinary Save investigation closed on `@322`. Conditional frozen-candidate replay and the complete final verdict remain required.
...
11. **Shared Lifecycle rollout — Houses · Income Sources plus Bills runtime proof**; Debt, Bank Accounts, and Investments are complete and Bills is source-complete. Shared Sheet Write Utilities and bound-deployment cleanup remain separate. *(The user-facing Tier-2 `getActiveSpreadsheet()` sweep closed by source inspection 2026-07-20.)*
...
Resume **Houses → Income Sources** after the core Rolling Financial Plan and complete the pending Bills runtime proof
