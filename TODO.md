...
4. **P3 Performance — Financial Plan refresh latency.** **Profiling complete; History/Dashboard chart retirement, Dashboard formatting batching, and snapshot construction isolated PASS.** Central `@116` preserved/appended `OUT - History` rows and completed History-backed snapshot comparisons while removing the six unused embedded History charts. Repeat History work fell from **11.507 s to 0.165 s** and total repeat server time from **77.275 s to 43.946 s**. `REG-054` passed isolated Central `@308`: one exact-workbook disposable pair completed at **38.614 s first / 40.605 s repeat**, all seven functional assertions passed, and Restricted sharing plus Trash cleanup were verified. `REG-056` passed isolated Central `@310`: exact production-path first/repeat runs passed all 14 functional and formatting read-back assertions, repeat `format_dashboard` fell from **18.303 s to 0.378 s**, and Restricted sharing plus Trash cleanup were verified. `REG-057` passed isolated `@317`: shared/batched supporting reads, selected-only Overview Retirement, single-grid Income readiness, and same-call canonical-position reuse cut current-position construction from **5.508 s to 0.128 s** and total snapshot from **13.026 s to 4.867 s**. Next run the exact-candidate 20-pair campaign while preserving correctness, email gating, and durable completion feedback.
