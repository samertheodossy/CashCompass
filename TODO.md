...
The **remaining P0 recovery-validation paths** are Auto-Adopt, Ambiguous, and Name-only adoption. Read-only Orphan detection is a separate P1 follow-up and does not block P0 closure — tracked in Stage 3 below. The **remaining Financial Integrity work** is the canonical basis + Planner/Dashboard/Rolling convergence + the Asset/Planner/Dashboard audit modules — also Stage 3. (Validation-surface note: the Script Properties UI may lag runtime changes during testing — **Admin Diagnostics is authoritative**.)
...
| Name-only confirmation/adoption validation (MEDIUM-confidence single candidate; OFF-confirm + ON-auto paths) | P0 | Disposable account | S |
...
> **Recovery/adoption defect (discovered 2026-07-07; P0 fix implemented 2026-07-16).** During Golden Workbook Convergence testing, **clearing a stale mapping let Central provision a *second* workbook even though another CashCompass workbook still existed in the user's Drive**. The fix makes candidate detection unconditional for no-mapping and stale-mapping resolution: confirmed zero is the only create path; HIGH/marker candidates relink; MEDIUM/name-only candidates require confirmation while auto-adopt is off; ambiguity and search/verify uncertainty stop safely. **The exact HIGH-marker Admin Clear/reload reproduction passed 2026-07-17 with no duplicate and the bounded workbook intact; the remaining decision-tree matrix stays open.**

> **P0/P1 disposition (2026-07-19).** P0 closes only after the remaining duplicate-prevention safety matrix is runtime-validated on a disposable Central account: confirmed-zero create, MEDIUM confirmation with auto-adopt OFF, MEDIUM auto-adopt with the flag ON, ambiguity, search/verification failure, stale variants, and cross-user isolation. Read-only Orphan detection remains P1 because it surfaces evidence without changing resolution or creation behavior; it does not block P0 closure. All recovery flags return OFF after the P0 pass.
...
- **6C.1 — Adopt-Before-Create / duplicate guard** *(✅ P0 decision tree implemented 2026-07-16; HIGH-marker clear/relink + bounded isolation runtime-validated 2026-07-17; remaining matrix pending)* — no-mapping and stale-mapping resolution always search under the user lock. Confirmed zero is the only create path; one HIGH/marker candidate relinks; one MEDIUM/name-only candidate relinks only when `CENTRAL_AUTO_ADOPT=true`, otherwise it routes to an explicit confirmation screen; ≥2 throws `AmbiguousWorkbookError`; search/verify failures route to unavailable and never create. Functions: `resolveExistingWorkbookForRecovery_`, `tryAdoptWorkbookBeforeCreate_`, `relinkSingleCandidate_`, `buildAmbiguousWorkbookError_`, `buildConfirmAdoptWorkbookError_`, `buildWorkbookRecoveryUnavailableError_`.
...
- **6F — Recovery Validation** *(current; P0 safety matrix + P1 orphan detection, ~94–96%)* — healthy-path load validated 2026-06-09; **recovery-page render from a real stale mapping + executed Reconnect validated 2026-06-11** (reconnect reload bug fixed); **executed Admin Clear Mapping + mapping/reverse-index removal + repair audit history + bootstrap reprovision after clear + Welcome routing + dashboard empty-state + Financial Integrity `NOT_INITIALIZED` handling + Central admin routing + `ADMIN_EMAILS` validated 2026-07-02**. Remaining P0 = confirmed-zero create, MEDIUM name-only confirmation with auto-adopt OFF + auto-adopt ON, ambiguity, search/verification failure, stale variants, and cross-user isolation on a disposable account; then **flags OFF**. Read-only Orphan detection remains P1 and does not block P0 closure. No new code expected unless a defect is found. See `## Open testing inventory`.
...
**Executed Admin Clear validation (2026-07-02):** with `CENTRAL_ADMIN_REPAIR=true`, an **executed Admin Clear Mapping** removed the mapping + reverse-index property and wrote the expected repair audit entry; the next load **reprovisioned** a fresh workbook (Welcome routing + dashboard empty-state correct), and the Financial Integrity Audit correctly reported `NOT_INITIALIZED` on the bootstrap-only workbook. Central admin routing (`?view=admin`) + `ADMIN_EMAILS` were also validated. Flag returned OFF afterward. Remaining P0 recovery paths (Auto-Adopt, Ambiguous recovery, and Name-only adoption) are still pending. Read-only Orphan detection is a separate P1 follow-up and does not block P0 closure — see `## Open testing inventory`.
...
Prior passes validated healthy mapped load, the recovery page, executed reconnect, disabled admin repair, and executed Admin Clear. The **P0 HIGH-marker clear/relink path passed across two accounts 2026-07-17** with the auto-adopt flag off, no duplicate, mapping/reverse index restored, Recovery suite PASS, and the bounded workbook intact. The `cashcompass2026@gmail.com` repetition recovered a distinctive `$10,000` value from the same existing workbook after mapping clear and reload. The remaining P0 disposable-account matrix covers confirmed-zero create, MEDIUM confirm/off + auto/on, ambiguity, search/verify failure, stale variants, formal cross-user isolation, and drive.file-limited failure handling. Read-only Orphan detection remains a separate P1 follow-up. Authoritative copy: `PROJECT_CONTEXT.md → Recovery Validation Inventory`.
...
**Not yet validated (remaining recovery items):**

- P0 decision-tree runtime matrix on a disposable Central account.
- Auto-Adopt validation (`CENTRAL_AUTO_ADOPT=true`, MEDIUM/name-only candidate).
- Ambiguous and unavailable failure routing.
- Name-only explicit confirmation with `CENTRAL_AUTO_ADOPT=false`.
- **P1 (non-blocking):** read-only Orphan workbook validation.

**Readiness note:** Existing recovery evidence remains valid, but Family Beta reliance on the new P0 no-duplicate tree requires the disposable runtime matrix. Not all remaining paths are gated by `CENTRAL_AUTO_ADOPT`; only automatic MEDIUM/name-only adoption is.
...
| Item | Flag | Risk | Status / timing |
| --- | --- | --- | --- |
| P0 duplicate-prevention decision tree | — | High | Local tests PASS; Apps Script scenario 7/7 + Recovery suite 1/1 PASS; HIGH-marker Admin Clear/relink + bounded isolation PASS; remaining disposable matrix pending. |
...
| Ambiguous recovery (≥2 → `AmbiguousWorkbookError`) | — | Medium | Not runtime-tested → 6F. |
| Name-only confirm/adopt (MEDIUM-confidence single candidate) | `CENTRAL_AUTO_ADOPT` | Medium | Decision implemented; OFF-confirm + ON-auto runtime paths pending. |
...
**6F — Recovery Validation pass:** run `SUITE-RECOVERY-REGRESSION` first, then exercise the full decision matrix on a **disposable Central account only**. Validate confirmed-zero create, HIGH relink with auto-adopt off, MEDIUM confirm/off + auto/on, ambiguity, search/verify failure, stale recovery, and no cross-user leakage. Return `CENTRAL_AUTO_ADOPT` and `CENTRAL_RECOVERY_ACTIONS` to `false` after validation and record evidence in `SESSION_NOTES.md`. Read-only Orphan detection is a separate P1 follow-up and is not a P0 exit criterion. No production/bounded workbook is involved.
