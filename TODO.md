...
2. **Exact-candidate browser evidence trust (`REG-015`; standalone fail-closed path runtime-proven, dedicated exact-owner proof pending).** Standalone Populated Dashboard PASS reports on isolated `@175`–`@177` inherited the parked `@141` `IN_PROGRESS` run and incorrectly saved release-eligible evidence. Generic/direct launches now carry no owner and are diagnostic-only even while an old run remains active. The final `@178` run first proved the fail-closed attribution. The related Debt timeout is fixed under `REG-017`: isolated `@179` run `FR-3f6f2cf7-f823-4b74-a033-5e964f66b05e` passed all 12 assertions with zero captured errors and verified Trash cleanup. Apps Script HTTP 0 recovery remains open. Only a dedicated action in the active Release Readiness table supplies its exact run id; the admin launcher validates that id, campaign preparation captures it, and completion revalidates it. Diagnostic Performance cannot ratify a release budget.
