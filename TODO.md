...
11. **Shared Lifecycle rollout — House metadata · Income Sources separately**; Debt, Bank Accounts, Investments, Bills, and House Stop/Reactivate are complete. Shared Sheet Write Utilities and bound-deployment cleanup remain separate. *(The user-facing Tier-2 `getActiveSpreadsheet()` sweep closed by source inspection 2026-07-20.)*
...
- **Assets → House Values — Inactive discovery + Reactivate** — **runtime-complete on isolated `@391`.** Manage houses exposes a counted inactive inventory backed by exact `SYS - House Assets` row/name identity and preserved House Values history. **`reactivateHouseFromDashboard`** takes the displayed SYS row plus expected exact name under a user lock; rejects moved/removed, renamed, duplicate/ambiguous, missing-history, missing-`HOUSES`-sheet, and inconsistent activation states; then changes only the matching House Values and House Assets **Active** cells to **Yes**, verifies the writes, and rolls back on failure. It preserves values, loan/type metadata, the `HOUSES - {House}` expense sheet, linked Debt property evidence, and prior Activity. Add continues to reserve active and inactive names across all three stores. **`house_reactivate`** is non-monetary Activity evidence with Action **Tracking resumed**. House Financial Accuracy passed 2/2 scenarios and 50/50 assertions; both Restricted fixtures were verified TRASHED and the runner returned OFF. House metadata Edit/Rename remains a separate `3o` task.
...
| House Values → Reactivate | **Runtime-complete on isolated `@391`** — counted inactive inventory in Manage; exact row/name stale and duplicate guards; only preserved Active cells return to Yes; values/assets/expense history/linked debt evidence survive; Add cannot create a duplicate inactive identity; `house_reactivate` shows **Tracking resumed** with Amount **—**; 2/2 scenarios and 50/50 assertions passed with both fixtures TRASHED and runner OFF. |
