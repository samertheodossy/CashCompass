...
2. **Exact-candidate browser evidence trust (`REG-015`; local fix implemented, runtime replay pending).** The standalone Populated Dashboard PASS executed on isolated `@175` was saved under stale candidate metadata `Central Apps Script version 141` / `isolated @141`. Browser campaigns now capture and revalidate the owning Release Readiness run; standalone evidence is explicitly diagnostic-only, and diagnostic Performance cannot ratify a release budget. Local P1 evidence regressions pass.
3. **Income/Setup cross-surface truth (`UX-03` + `UX-08`; `REG-016`; local fix implemented, interactive replay pending).** Setup reported one recurring income and zero other detected while Income reported no recurring income and showed the same salary as Other detected. Both surfaces now use one shared one-positive-month tracked-income classifier; excluded and negative/non-positive groups remain Other detected. The permanent Populated Dashboard browser suite now requires an explicit cross-surface consistency assertion in addition to local behavior/static regressions.
