...
- `VALIDATOR_ARCHITECTURE.md` — the read-only **Validator** subsystem (admin-gated, default-off): Golden↔Central formatting parity, the **recommendation engine** (AdoptGolden / KeepCentral / ProductDecision / IgnoreNoise), the scoped family runners, the Phase 1→2 roadmap, and the **Phase 2 Workbook Health** architecture (§10). Phase 1 complete (drove the convergence milestone); Phase 2 **Provisioning + Workbook Drift + Schema Evolution V1 implemented** (Formula / Conditional-Formatting / Named-Range validation + the aggregate Workbook Health report still ahead).
- `TEST_HARNESS_ARCHITECTURE.md` — the developer-only **Test Harness / Regression Runner** (the writer counterpart; default-off): disposable-workbook lifecycle, fail-closed target guard, the scenario model (SMOKE / REGRESSION / RECOVERY / STRESS), packs, and implementation order. Drives scenarios and calls the read-only Validator to judge health. **Foundation V1 implemented** — one SMOKE scenario runnable from the editor (`testRunSmoke()` / `testRunSmokeTrash()`) and from the Validation & Testing console; packs remain.
...
- `VALIDATION_TESTING_CONSOLE.md` — the admin-only **Validation & Testing** page (route `?view=validation`): one operator console for Validator / Workbook Health checks, Test-Harness suites, and the Release Readiness verdict, with an explicit per-target safety readout. Server functions return structured report objects; editor runners stay secondary. **V1 implemented** — Target + Workbook Health (Provisioning / Drift / Schema Evolution) + a single-scenario Test Harness card; multi-suite selector, Formula/CF buttons, and the Release Readiness verdict remain future.
...
- **Current active-work queue (near-term, 2026-07-09):** (1) **Bills Due Pay runtime validation** (confirm the shipped Pay occurrence bridge on the next real paid bill), (2) **Investments cleanup** (finish in-flight convergence + clear `investments.js` WIP), (3) **Recovery implementation** (ratified Recovery Architecture — unconditional candidate detection, Auto-Adopt / Ambiguous / Name-only / Orphan, duplicate guard; `CENTRAL_APP_RECOVERY_COMPLETION_PLAN.md`), (4) **Validation Agent** (automated release gate), (5) **Golden Workbook polish** (converge provisioning toward the Golden Workbook; first audit complete), (6) **Family Beta** (UX polish + Beta Gate assembly + go/no-go).
- **Beta Gate criteria (unordered requirements — all must pass before Family Beta):** Golden Workbook Convergence · **Financial Integrity Phase 3** (canonical basis + Planner/Dashboard/Rolling convergence to $0.01 + Asset/Planner/Dashboard audit modules — *the numbers must reconcile*) · Validation Agent · Recovery completion · Workbook/UX polish. The execution-order queue above sequences these. Remaining recovery slices (Stage 4): 6D.2b Create New Workbook, 6E.2 Admin Set Mapping.
...
- **Stage 3 — Beta Readiness** *(current)* — the **Beta Gate**, sequenced by the **confirmed execution order (2026-07-09)**: **(1) Bills Due Pay runtime validation** (confirm the shipped Pay occurrence bridge, then commit), **(2) Investments cleanup** (finish in-flight convergence + commit WIP), **(3) Recovery completion — remaining adoption paths** (ratified Recovery Architecture: unconditional candidate detection + Auto-Adopt / Ambiguous / Name-only / Orphan + duplicate guard), **(4) Validation Agent** (automated release gate that protects the reconciled numbers), **(5) Golden Workbook polish** (P0/P1, L — converge provisioning toward the **Golden Workbook** / visual source of truth; **first audit complete 2026-07-06** — ten core sheets across four design families; see `GOLDEN_WORKBOOK.md` + `WORKBOOK_PARITY_CHECKLIST.md`; convergence, not redesign), **(6) Family Beta polish** (Workbook / UX polish + release/onboarding review). **Financial Integrity — Phase 3** (canonical basis + Planner/Dashboard/Rolling convergence to $0.01 + Asset/Planner/Dashboard audit modules — *the numbers must reconcile*) is a Beta-Gate requirement sequenced alongside (3)–(5). Goal: **Family Beta Release Candidate.**
- **Stage 4 — Family Beta** — go/no-go, Web App UX polish + loading-experience consistency, recovery slices (6D.2b / 6E.2), Shared Lifecycle rollout (Bank Accounts), feedback, workflow stabilization.
- **Stage 5 — External Beta** — **Money Plan Phase 2** (long-term/goal planning, recommendations, forecasting — major V1 differentiator) + **Income Expected/Due Workflow**; support, feedback, invite onboarding, user-lifecycle handling, scalability, remaining Shared Lifecycle rollout (Investments / Houses / Income Sources), Shared Sheet Write Utilities, Tier-2 migration cleanup.
