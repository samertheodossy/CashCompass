...
- `VALIDATOR_ARCHITECTURE.md` — the read-only **Validator** subsystem (admin-gated, default-off): Golden↔Central formatting parity, the **recommendation engine**, scoped family runners, and **Phase 2 Workbook Health**. The shipped foundation is Provisioning + Workbook Drift + Schema Evolution V1; Formula, Conditional Formatting, Named Ranges, formal schema registry, and aggregate Workbook Health are pushed to Central and isolated at `@120`, with dedicated runtime validation pending.
- `TEST_HARNESS_ARCHITECTURE.md` — the developer-only **Test Harness / Regression Runner** (the writer counterpart; default-off): disposable-workbook lifecycle, fail-closed target guard, the scenario model (SMOKE / REGRESSION / RECOVERY / STRESS), packs, and implementation order. Drives scenarios and calls the read-only Validator to judge health. **Foundation V1 + populated-fixture hardening implemented** — Central-created synthetic fixtures are Restricted, cover eight representative data domains, and use verified recoverable Trash cleanup without touching owner/bounded workbooks. Seven suites are runtime-proven, including isolated `@120` Performance Planner and Bills Pay E2E; First-Run UX E2E, Populated Dashboard E2E, and Recovery Live remain fail-closed pending real execution seams.
...
- `RELEASE_READINESS.md` — the pre-release **go/no-go report** format + bounded workflow (`Release Readiness gate = Harness × Validator`); local runner executes one disposable scenario per invocation and archives compact evidence, but remains NOT READY pending three execution-dependent suites and runtime proof.
- `VALIDATION_TESTING_CONSOLE.md` — the admin-only **Validation & Testing** page (route `?view=validation`): one operator console for Validator / Workbook Health checks, Test-Harness suites, and the Release Readiness verdict, with an explicit per-target safety readout. Server functions return structured report objects; editor runners stay secondary. **V1 implemented** — Target + Workbook Health (Provisioning / Drift / Schema Evolution) and five runnable Test-Harness suites, including Central Safety. Formula/CF buttons and the bounded aggregate Release Readiness verdict remain future.
...
- **Current active work:** P0 Project Stabilization closed after isolated `@114` passed both blank/fresh and representative populated-workbook validation. Populated-fixture hardening passed on `@117`. The P1 source slice is now isolated at Central `@120`: Bills Pay E2E passed 3/3 with verified Trash, and Performance Planner passed 4/4 with 32.779 s first / 31.901 s repeat timings, retained History, zero History charts, Restricted/CURRENT/Provisioning/Drift/Trash PASS. Beta remains pinned to `@106`; no bound deployment or owner/bounded workbook was touched. Remaining P1 work is dedicated Workbook Health runtime evidence, First-Run UX E2E, Populated Dashboard E2E, Recovery Live, performance-budget ratification, console controls, and the final bounded Release Readiness verdict. P2 financial truth and beta-critical P3 UX/performance follow. The quality baseline is 7.5/10; broad or monetized beta waits for the ≥95/100 gate in `BETA_10_OUT_OF_10_PLAN.md`. Bills Due → Pay natural cohort evidence remains pending; do not create fake financial activity to force it.
