...
- `RELEASE_READINESS.md` — the pre-release **go/no-go report** format + bounded workflow (`Release Readiness gate = Harness × Validator`); local runner executes one disposable scenario per invocation and archives compact evidence, but remains NOT READY pending Recovery Live, performance-budget ratification, and final orchestration proof.
