...
## 0. Current phase — Stage 3: Beta Readiness

**Stage 1 (Core Platform) and Stage 2 (Product Hardening) are complete.** The Central App architecture is live (CENTRAL_MODE routing, per-user provisioning, workbook mapping) and family-beta capable. **Product Hardening delivered:** the Recovery **destructive/admin paths validated** (executed Admin Clear + mapping/reverse-index removal + repair audit history + bootstrap reprovision + Welcome routing + empty-dashboard + recovery routing + Reconnect + Central admin validation + `ADMIN_EMAILS`; *recovery validation completed 2026-07-02*); the **Financial Integrity foundation** (Audit Framework + Debt Audit + shared debt Active helper + `NOT_INITIALIZED`); and the **Bills Due performance optimization (~51s → ~5.6s)**. The **only remaining recovery-validation paths** are Auto-Adopt, Ambiguous, Name-only adoption, and Orphan.

The current stage is the **Beta Gate**, sequenced by the **confirmed execution order (2026-07-09)**: (1) **Bills Due Pay runtime validation** (confirm the shipped Pay occurrence bridge, then commit), (2) **Investments cleanup** (finish in-flight convergence + commit WIP), (3) **Recovery completion — remaining adoption paths** (ratified Recovery Architecture), (4) **Validation Agent** (the automated release gate that protects the reconciled numbers), (5) **Golden Workbook polish** (converge provisioning toward the visual source of truth; first audit complete), (6) **Family Beta polish** (Workbook / UX polish). **Financial Integrity — Phase 3 (convergence)** (canonical basis + Planner/Dashboard/Rolling convergence + Asset/Planner/Dashboard audit modules — *the numbers must reconcile*) is a Beta-Gate requirement sequenced alongside (3)–(5). The authoritative roadmap is **`TODO.md → Product Maturity Stages`** (Stage 1–6; mirror in `PROJECT_CONTEXT.md`); ranked priorities in `TODO.md → Current Engineering Priorities` / `→ Known priorities (ranked)`; flags in `PROJECT_CONTEXT.md → Flag Registry`; untested recovery items in `TODO.md → Open testing inventory`. Product framing is in `PROJECT_CONTEXT.md → Current architecture — Central App (live)`, `→ Workbook Identity & Recovery (live + roadmap)`, and `→ Family Beta workbook styling`.
...
Authoritative roadmap lives in `TODO.md → Product Maturity Stages` (Stage 1–6 + `Current Engineering Priorities` + `Beta Gate`; high-level mirror in `PROJECT_CONTEXT.md`). **Current stage: Stage 3 — Beta Readiness** (Stages 1–2 complete; Family Beta ~97–98% · External Beta ~92%; remaining work is the Beta Gate — in agreed order: Golden Workbook Convergence, Financial Integrity Phase 3, Validation Agent, Recovery completion, Workbook/UX polish — not major feature development). Mirror here is short on purpose:

- **Active-work queue (near-term, 2026-07-09):** (1) **Bills Due Pay runtime validation** — confirm the shipped Bills Due → Pay occurrence bridge on the next genuine paid bill (marker written, occurrence disappears, no duplicates); (2) **Investments cleanup** — finish in-flight Investments convergence + clear `investments.js` WIP; (3) **Recovery implementation** — build the ratified Recovery Architecture (`CENTRAL_APP_RECOVERY_COMPLETION_PLAN.md`: unconditional candidate detection, Auto-Adopt / Ambiguous / Name-only / Orphan, provision-after-recovery duplicate guard); (4) **Validation Agent** — automated regression-detection release gate; (5) **Golden Workbook polish** — converge fresh provisioning toward the Golden Workbook (first audit complete 2026-07-06, ten sheets / four design families; `GOLDEN_WORKBOOK.md` + `WORKBOOK_PARITY_CHECKLIST.md`); (6) **Family Beta** — UX polish sweep + Beta Gate assembly + go/no-go.
- **Beta Gate criteria (unordered — all must pass before Family Beta):** Golden Workbook Convergence · **Financial Integrity Phase 3** (canonical basis + Planner/Dashboard/Rolling convergence to $0.01 + Asset/Planner/Dashboard audit modules — *the numbers must reconcile*) · Validation Agent · Recovery completion · Workbook/UX polish. The active-work queue above sequences these; Financial Integrity Phase 3 is sequenced alongside Recovery / Validation Agent even though it sits outside the numbered queue. Full detail: `TODO.md → Product Maturity Stages → Stage 3`.
- **Delivered (headline):** Central App migration (resolver + provisioning + mapping, live); Family Beta styling for Bank Accounts / Debts / Bills / Upcoming Expenses; read-only admin diagnostics (Phase 2A); Workbook Totals parity (TOTAL DEBT 3.1, Bank Accounts Total Accounts 3.2a + Delta 3.2b); Bank/Debts Add-New dropdown merge fix; Workbook Identity markers (6A/6B); **Workbook Recovery stack (6C.1 / 6D.1 / 6D.2a / 6E.1) shipped behind flags with destructive/admin paths validated (Admin Clear + mapping/reverse-index removal + repair audit + bootstrap reprovision + Welcome routing + empty-dashboard + Reconnect + Central admin + `ADMIN_EMAILS`); Auto-Adopt / Ambiguous / Name-only / Orphan pending (6F)**; **Financial Integrity Audit Framework + Debt Audit + shared debt Active helper + `NOT_INITIALIZED`**; **Bills Due performance optimization (~51s → ~5.6s)**; **Recurrence Engine V2 — Weekday & Biweekly scheduling** (weekday-aware Weekly + anchor-driven Biweekly, `Weekday`/`Anchor Date`/`Schedule Effective Date` columns, prospective-only schedule changes, AutoPay `LockService` hardening); **Bills Due → Pay occurrence bridge**. *(Label note: "6A–6F" expands macro Phase 2B; it is not macro "Phase 6 — External Beta Readiness," now Stage 5.)*
- **UX Backlog (Version 1):** the permanent home for small product-quality improvements (loading standardization, empty-state standardization, UX consistency framework, workbook visual parity, Money Plan Phase 2, **Bills Scheduling UX** — field-order polish / inline Anchor Date validation / derived Schedule column / helper-text review / formatting parity review) is **`TODO.md → UX Backlog (Version 1)`**. Intentionally separated from engineering priorities; completed opportunistically; **reviewed before every Family Beta milestone and before External Beta**; cross-referenced from the Stage roadmap so items never disappear during a reorganization. Append new polish/consistency/loading/empty-state/wording findings there.
...
- **Later (post-V1.2 / future phase):** onboarding factory refactor, Activity smart-undo Phases 2–4, Cash Strategy, HELOC advisor refinement, Plaid-style sync, broader regression / test harness, two-dashboards unification, **Merge Debt Accounts** (see below), **Debt Payee Aliases** (see below), **Google Sheets Refresh Awareness** (see below), **Shared Entity Lifecycle Framework** (see below), **Shared Sheet Write Utilities** (see below), and the other big-product items captured below and in `TODO.md → Historical backlog`.
...
Authoritative copy: `TODO.md → Future UI Standardization — Manage Pattern Rollout` (high-level mirror in `PROJECT_CONTEXT.md`). **Status:** documented, **not implemented.** **Bills = complete · Debts = complete · Bank Accounts = next likely candidate.**
...
The **lifecycle** companion to Manage Pattern Rollout above. **Long-term goal:** all tracked entities eventually share the same lifecycle — `Create → Edit → Rename → Stop Tracking → Inactive → Reactivate` — across long-lived entities (Debts *(reference)*, Bank Accounts, Investments, Houses, Bills, Income Sources) so each module stops reinventing its own lifecycle UI, labels, colors, confirm dialogs, activity logging, and server guardrails. Shared pieces to extract: UI language (Active / Inactive sections, "Show inactive" toggle with count, Reactivate, Danger Zone, Stop Tracking, common empty state + helper text); button styles (Edit / Rename neutral, Reactivate positive/green, Stop Tracking destructive/red); confirmation copy (remains in history · excluded from live calcs · restorable from Inactive); server lifecycle helpers (deactivate / reactivate existing row, block duplicate active names, preserve history, allow-list guard preventing direct `Active` edits via generic update endpoints, activity events); activity event pattern (`<entity>_deactivate` / `<entity>_reactivate` → "Tracking stopped" / "Tracking resumed", consistent JSON details); and lifecycle diagnostics (inactive-with-balance, duplicate active names, inconsistent `Active` values). Phased: document Debt as reference → inventory other modules → extract CSS/copy → extract server helpers → migrate one module at a time with runtime validation. **Priority: Medium** — after Financial Integrity reconciliation, before broader external beta if time allows. Not a blocker.

### Future — Shared Sheet Write Utilities (write-path standardization)

Authoritative copy: `TODO.md → Future Feature — Shared Sheet Write Utilities` (high-level architectural note in `PROJECT_CONTEXT.md`). **Status:** documented, **not implemented.** **Priority: Medium** — after Financial Integrity, Recovery Validation, and the Validation Agent; before broader public beta. Not a blocker.

The **data-write** companion to the Shared Entity Lifecycle Framework above (same Shared Component direction, applied to how values land in the sheet). Many modules now write back to Google Sheets — Bills, Debt updates, Bank Accounts, Investments, Houses, House Expenses, Cash Flow, Planner outputs — and each re-implements its own "set value + fix formatting" logic, so formatting-preservation bugs keep recurring (e.g. the Debt **Credit Limit** cell losing its green font when the write pasted a row neighbor's format; blank Cash Flow autopay cells rendering `-3` instead of red `-$3.00`). **Long-term goal:** a shared Sheet Write utility layer so all writes behave consistently and a formatting fix made once applies everywhere. Possible helpers: `writeCellPreserveFormatting_()`, `writeRangePreserveFormatting_()`, `updateCurrencyCell_()`, `updatePercentCell_()`, `updateDateCell_()`. Responsibilities: preserve number format, font color, font weight, alignment, borders, and background where appropriate; preserve formulas unless intentionally replaced; centralize currency + percentage formatting; reduce duplicated write logic. **Design principle:** business logic decides *what* to write, shared helpers decide *how* — and "preserve the cell's own look" (per-column, e.g. Debts) vs "match the row" (row-uniform, e.g. Cash Flow) must be explicit named behaviors, keeping the two number conventions (black vs `[Red]` negatives) separable. Existing helpers to rationalize: `applyCurrencyFormat_`, `setCurrencyCellPreserveRowFormat_`, `copyNeighborFormatInRow_`, `copyNearestAmountFormatInRow_`, the `setCashFlowMoney*` family (`planner_helpers.js`), `writeDashboardBillValuePreserveFormat_` (`dashboard_data.js`), and the `PASTE_FORMAT` template-row copy in `bank_accounts.js`. **Phased — do NOT refactor immediately:** inventory write helpers → identify duplicated formatting → introduce shared helper(s) → migrate modules one at a time as they are touched, each with runtime validation. UX/maintainability enhancement, not a blocker.

### Future — Income Expected / Due Workflow (post-Central stabilization)

Authoritative copy: `TODO.md → Future Feature — Income Expected / Due Workflow` (high-level mirror in `PROJECT_CONTEXT.md`). **Status:** documented, **not implemented.** **Roadmap placement: Stage 5 — External Beta, P2, L** — a first-class Version 1 product differentiator (income symmetry with Bills Due), restored from being folded into the Version 2 catch-all.
...
A dedicated **top-level Money Plan page** built on the **10/70/20** model (Save 10% / Living 70% / Debt Payoff 20%). The dashboard keeps a **summary card only** (Income, Save / Living / Debt targets, Status, **Open Money Plan** button). The page is **read-only and fully derived from existing data** — no new sheets, no writes, no auto-created transfers / payments / debt actions. Calc model: income (same basis as the dashboard card); **debt actual** = Cash Flow expense rows matched to active debt accounts (reuse `buildAnchorMonthCfPaidByDebtName_` / `expensePayeeMapFromRawRows_`); **living actual** = total Cash Flow expenses − debt actual; **savings actual headline = residual** (`income − living − debt`). Investment-account movement is deliberately **not** the headline savings figure (market changes distort it). Page eventually shows: current-month plan vs actual; Save / Living / Debt cards; status badge (on track / warning / off track); recommendations; month history table; and selectable strategy settings (10/70/20, 50/30/20, custom — later sub-phase). UX enhancement, not a blocker.

### Future — Debt Payee Aliases (post-Central stabilization)

Authoritative copy: `TODO.md → Future Enhancement — Debt Payee Aliases` (high-level mirror in `PROJECT_CONTEXT.md`). **Status:** documented, **not implemented.** **Long-term enhancement — not part of current Central stabilization work.**

Automatic debt payment detection requires the Cash Flow payee to **normalize-match the tracked debt Account Name exactly** (`normalizeBillName_`), so a payment payee of `Meriwest` is never attributed to the debt `Meriwest Credit Union Loan` — the exact mismatch surfaced during the Meriwest HELOC payment investigation (the payee never reached the matched-debt branch in `adjustDebtsBalanceAfterQuickPayment_`). The enhancement lets each tracked debt carry **one or more optional payment aliases** (e.g. `Meriwest`, `Meriwest CU`, `Meriwest HELOC`) used **only for payment recognition and reporting**. Future uses: debt payment recognition, Loan/HELOC informational messaging, a future principal-payment workflow, Rolling Debt payoff attribution, Money Plan debt-actual calculations, imported bank-transaction matching, and bank import reconciliation. **Design guardrails:** Account Name stays the canonical identifier; aliases optional; matching stays **deterministic** (normalized exact alias match — no fuzzy/AI); multiple aliases per debt supported; existing behavior preserved when no aliases are configured. Likely a new optional `Aliases` column on `INPUT - Debts` (provisioning / header-repair review required before implementation). UX enhancement, not a blocker.

### Future — Google Sheets Refresh Awareness (post-Central stabilization)

Authoritative copy: `TODO.md → Future UX Enhancement — Google Sheets Refresh Awareness` (high-level mirror in `PROJECT_CONTEXT.md`). **Status:** documented, **not implemented.** **Priority: Low — UX only, not a Central architecture blocker, not a data-integrity issue.**

When CashCompass writes to the workbook via Apps Script (House Expenses, Quick Add, Bills Due, AutoPay, etc.), an **already-open native Google Sheets browser tab** may not display the updated row/cell until it is refreshed. The data is written correctly — the web app and server-side reads return the right values — so this looks like **Google Sheets client/UI caching**, not a CashCompass write failure. Future ideas (none committed): investigate whether `SpreadsheetApp.flush()` or existing flush timing improves live visibility; consider `SpreadsheetApp.toast()` or another Sheets-side notification; consider a lightweight post-write web-app message (*"Saved successfully. If your Google Sheet is already open, you may need to refresh it to see the latest changes."*); investigate whether any write path can trigger a Sheets client refresh without a full browser reload. **Do not** introduce artificial delays or repeated flushes unless a measurable benefit is demonstrated. Pursue only after Central stabilization and broader beta testing.

### Delivered — Recurrence Engine V2: Weekday & Biweekly scheduling (2026-07-09)

True weekday-aware recurrence for Weekly and Biweekly bills (commit *Add biweekly weekday scheduling*), superseding the earlier "Tier 2 — Weekly/Biweekly weekday recurrence" future item. Delivered as separate **`Weekday`** + **`Anchor Date`** columns (not a single `Repeat Day`), plus a **`Schedule Effective Date`** column for prospective-only changes.

- **Pure recurrence refactor** — `buildRuleFromBillRow_` + `generateOccurrences_` seam (byte-identical legacy output).
- **Weekly weekday** — continuous 7-day cadence across month boundaries.
...
- **Schema evolution** — `Weekday` / `Anchor Date` / `Schedule Effective Date` on `INPUT - Bills`; new-workbook provisioning + append-only self-heal (`ensureBillsSheetSchema_`) + formatting parity.
- **UI** — `Weekday` for Weekly + Biweekly; `Anchor Date` for Biweekly only; Anchor Date convenience prefill.
- **Validation** — Anchor Date must fall on the selected Weekday (`isAnchorDateValidForWeekday_`), client + server, **no silent correction**.
- **Prospective schedule changes** — scheduling edits stamp `Schedule Effective Date = today`; earlier occurrences never generate/autopay (no retroactive Cash Flow, no history rewrite).
- **AutoPay concurrency hardening** — `LockService` guards autopay writes (exactly-once).

Backward compatible (blank fields → legacy Due Day; Monthly/other frequencies unchanged); Cash Flow, Activity Log format, and AutoPay/Pay/Skip semantics preserved. Runtime validated (Laith, Lutfi, M1, Robinhood, San Jose House Cleaning). Files: `dashboard_data.js`, `bills.js`, `onboarding.js`, `Dashboard_Body.html`, `Dashboard_Script_BillsDue.html`. Small non-blocking follow-ups tracked in `TODO.md → UX Backlog → Bills Scheduling UX`.
...
8. **Weekly/Biweekly weekday recurrence. ✅ DELIVERED (2026-07-09) — Recurrence Engine V2.** Shipped as separate `Weekday` + `Anchor Date` columns (not a single `Repeat Day`), plus `Schedule Effective Date` for prospective-only changes. See **`### Delivered — Recurrence Engine V2: Weekday & Biweekly scheduling`** above and `PROJECT_CONTEXT.md → Weekly/Biweekly Weekday Recurrence Support — DELIVERED`.
