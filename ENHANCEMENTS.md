...
- **Later (post-V1.2 / future phase):** onboarding factory refactor, Activity smart-undo Phases 2–4, Cash Strategy, HELOC advisor refinement, Plaid-style sync, broader regression / test harness, two-dashboards unification, **Merge Debt Accounts** (see below), **Debt Payee Aliases** (see below), **Google Sheets Refresh Awareness** (see below), and the other big-product items captured below and in `TODO.md → Historical backlog`.
