...
- **Active now (Stage 3 — Beta Gate):** (1) **Validation Agent** — automated regression-detection release gate (highest engineering priority); (2) **Financial Integrity convergence** — canonical basis + Planner/Dashboard/Rolling convergence to $0.01 + Asset/Planner/Dashboard audit modules (foundation shipped); (3) **Workbook Formatting & Visual Parity** — newly provisioned workbook visually indistinguishable from production, the **Golden Workbook** / visual source of truth (P1, L — a Family Beta quality gate, not minor polish); **first task = Golden Workbook Audit** (production screenshots) before any styling code — spec `GOLDEN_WORKBOOK.md`, checklist `WORKBOOK_PARITY_CHECKLIST.md`; (4) **Recovery Validation — remaining adoption paths** (6F: Auto-Adopt / Ambiguous / Name-only adoption / Orphan, then flags OFF).
