...
The app has moved past "V1.2 / controlled improvement mode." **The Central App architecture is live** (CENTRAL_MODE routing, per-user workbook provisioning, workbook mapping) and is **stable and family-beta capable**; Family Beta styling shipped for four input sheets. Recently completed: read-only admin diagnostics (Phase 2A), Workbook Totals parity (TOTAL DEBT 3.1, Bank Accounts Total Accounts 3.2a + Delta 3.2b), the Bank/Debts Add-New dropdown fix, and Workbook Identity markers (Phase 6A design + 6B markers). The authoritative forward plan is the detailed **`Launch Readiness Roadmap`** in `TODO.md` (high-level summary + ranked priorities in `PROJECT_CONTEXT.md` and `TODO.md → Known priorities (ranked)`); product framing is in `PROJECT_CONTEXT.md → Current architecture — Central App (live)`, `→ Workbook Identity & Recovery (live + roadmap)`, and `→ Family Beta workbook styling`.
...
- **Active now:** Phase 2 — Family Beta Hardening — **2B Workbook Recovery**, expanded as the **Workbook Identity & Recovery (6A–6E)** track. 6A design ✅ and 6B identity markers ✅ are done; the next efforts are **6C Adopt-Before-Create → 6D Recovery UX → 6E Admin Repair Tools**. Design: `CENTRAL_APP_WORKBOOK_DIAGNOSTICS_PLAN.md` + the Phase 6A design pass. *(Label note: "6A–6E" expands macro Phase 2B; it is not macro "Phase 6 — External Beta Readiness.")*
- **Delivered (headline):** Central App migration (resolver + provisioning + mapping, live); Family Beta styling for Bank Accounts / Debts / Bills / Upcoming Expenses; read-only admin diagnostics (Phase 2A); Workbook Totals parity (TOTAL DEBT 3.1, Bank Accounts Total Accounts 3.2a + Delta 3.2b); Bank/Debts Add-New dropdown merge fix; Workbook Identity markers (Phase 6A design + 6B markers).
