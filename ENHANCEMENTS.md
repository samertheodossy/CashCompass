...
- **Active now (Stage 3 — Beta Gate):** (1) **Validation Agent** — automated regression-detection release gate (highest priority); (2) **Financial Integrity convergence** — canonical basis + Planner/Dashboard/Rolling convergence to $0.01 + Asset/Planner/Dashboard audit modules (foundation shipped); (3) **Recovery Validation — remaining adoption paths** (6F: Auto-Adopt / Ambiguous / Name-only adoption / Orphan, then flags OFF).
