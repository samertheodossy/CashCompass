...
The app has moved past "V1.2 / controlled improvement mode." **The Central App architecture is live** (CENTRAL_MODE routing, per-user workbook provisioning, workbook mapping) and is **stable and family-beta capable**; Family Beta styling shipped for four input sheets. Recently completed: read-only admin diagnostics (Phase 2A), Workbook Totals parity (TOTAL DEBT 3.1, Bank Accounts Total Accounts 3.2a + Delta 3.2b), the Bank/Debts Add-New dropdown fix, and Workbook Identity markers (Phase 6A design + 6B markers). The **Workbook Recovery stack (6C.1 Adopt-Before-Create, 6D.1 Recovery Page, 6D.2a Reconnect, 6E.1 Admin Inspect + Clear Mapping) is implemented and committed, flag-gated OFF (`CENTRAL_AUTO_ADOPT`, `CENTRAL_RECOVERY_ACTIONS`, `CENTRAL_ADMIN_REPAIR`), and healthy-path validated (2026-06-09 — recovery + admin-repair flags ON, auto-adopt OFF: dashboard loads, existing workbook resolves, no recovery page, no regression, Repair Toolkit + Inspect/preview/confirm UI work); reconnect validated end-to-end (2026-06-11); and the **executed Admin Clear Mapping** path validated end-to-end (2026-07-02 — mapping/reverse-index removal, repair audit history, bootstrap reprovision after clear, Welcome routing, dashboard empty-state, Financial Integrity `NOT_INITIALIZED`, Central admin routing, `ADMIN_EMAILS`); the remaining paths (Auto-Adopt, ambiguous recovery, name-only adoption, orphan workbook) are not yet validated** — the active step is **Recovery Validation (6F)**. The authoritative forward plan is the detailed **`Launch Readiness Roadmap`** in `TODO.md` (high-level summary + ranked priorities in `PROJECT_CONTEXT.md` and `TODO.md → Known priorities (ranked)`; flags in `PROJECT_CONTEXT.md → Flag Registry` and `TODO.md → Flag Registry`; untested items in `TODO.md → Open testing inventory`); product framing is in `PROJECT_CONTEXT.md → Current architecture — Central App (live)`, `→ Workbook Identity & Recovery (live + roadmap)`, and `→ Family Beta workbook styling`.
...
Authoritative roadmap lives in `TODO.md → Product Maturity Stages` (Stage 1–5 + `Current Engineering Priorities` + `Beta Gate`; detailed Phase 1–7 in `TODO.md → Launch Readiness Roadmap`; high-level mirror in `PROJECT_CONTEXT.md`). **Current stage: Stage 2 — Product Hardening** (Family Beta ~96–97% · External Beta ~90–92%; remaining work is financial integrity, validation, recovery hardening, regression prevention, and UX polish — not major feature development). Mirror here is short on purpose:

- **Active now:** Phase 2 — Family Beta Hardening — **2B Workbook Recovery**, expanded as the **Workbook Identity & Recovery (6A–6F)** track. 6A design ✅, 6B identity markers ✅, and the **recovery stack (6C.1 / 6D.1 / 6D.2a / 6E.1) is implemented and committed, flag-gated OFF, healthy-path validated (2026-06-09), reconnect-validated (2026-06-11), and executed-Admin-Clear-validated (2026-07-02) — Auto-Adopt / ambiguous / name-only adoption / orphan workbook still pending**. The active step is **6F Recovery Validation** (remaining adoption/ambiguous/orphan pass, then flags OFF); remaining slices are **6D.2b Create New Workbook** and **6E.2 Admin Set Mapping**. Design: `CENTRAL_APP_WORKBOOK_DIAGNOSTICS_PLAN.md` + the Phase 6A design pass. *(Label note: "6A–6F" expands macro Phase 2B; it is not macro "Phase 6 — External Beta Readiness.")*
- **Delivered (headline):** Central App migration (resolver + provisioning + mapping, live); Family Beta styling for Bank Accounts / Debts / Bills / Upcoming Expenses; read-only admin diagnostics (Phase 2A); Workbook Totals parity (TOTAL DEBT 3.1, Bank Accounts Total Accounts 3.2a + Delta 3.2b); Bank/Debts Add-New dropdown merge fix; Workbook Identity markers (Phase 6A design + 6B markers); **Workbook Recovery stack shipped behind flags (default OFF; healthy-path + reconnect + executed-Admin-Clear validated; Auto-Adopt / ambiguous / name-only adoption / orphan workbook pending 6F): 6C.1 Adopt-Before-Create, 6D.1 Recovery Page, 6D.2a Reconnect, 6E.1 Admin Inspect + Clear Mapping.**
...
Authoritative copy: `TODO.md → Future Feature — Income Expected / Due Workflow` (high-level mirror in `PROJECT_CONTEXT.md`). **Status:** documented, **not implemented.** **Priority: Medium-high, after current Central stabilization.**
...
Authoritative copy: `TODO.md → Future Feature — Money Plan (Income Allocation) Page` (high-level mirror in `PROJECT_CONTEXT.md`). **Status: Phase 1 dashboard 10/70/20 card implemented (pending commit); Phase 2 Money Plan page = documented design direction, not implemented — separate approval required.**
