...
The current stage is the **Beta Gate**, in agreed priority order: (1) **Golden Workbook Convergence** (converge provisioning toward the visual source of truth; first audit complete), (2) **Financial Integrity — Phase 3 (convergence)** (canonical basis + Planner/Dashboard/Rolling convergence + Asset/Planner/Dashboard audit modules — *the numbers must reconcile*), (3) **Validation Agent** (the automated release gate that protects the reconciled numbers), (4) **Recovery completion — remaining adoption paths**, (5) **Workbook / UX polish**. The authoritative roadmap is **`TODO.md → Product Maturity Stages`** (Stage 1–6; mirror in `PROJECT_CONTEXT.md`); ranked priorities in `TODO.md → Current Engineering Priorities` / `→ Known priorities (ranked)`; flags in `PROJECT_CONTEXT.md → Flag Registry`; untested recovery items in `TODO.md → Open testing inventory`. Product framing is in `PROJECT_CONTEXT.md → Current architecture — Central App (live)`, `→ Workbook Identity & Recovery (live + roadmap)`, and `→ Family Beta workbook styling`.
