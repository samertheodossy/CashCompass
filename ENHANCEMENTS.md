...
- **Completed populated-fixture follow-up:** isolated Central `@117` verified Central-created, `drive.file`-compatible fixtures; representative Bank, Investment, House, Debt, Bills, Income, Upcoming, and Retirement seeding; Restricted sharing; 9/9 functional assertions; Validator checks; and Drive-confirmed Trash cleanup without touching owner/bounded workbooks. P2 UX polish owns consistent **Due day of month** debt wording and a non-ambiguous **Checking…** state for optional Setup cards. The measured 143-second Financial Plan refresh remains P3 Performance. These do not reopen P0.
...
- **Elevated to broad-Beta gate:** Activity correction/reversal (`FULL_BETA_REMAINING_PLAN.md → 5g–5m`; safety contract in `TODO.md → Activity — Correction / reverse transaction`).
- **Later (post-V1.2 / future phase):** onboarding factory refactor, Cash Strategy, HELOC advisor refinement, Plaid-style sync, broader regression / test harness, two-dashboards unification, **Merge Debt Accounts** (see below), **Debt Payee Aliases** (see below), **Google Sheets Refresh Awareness** (see below), **Shared Entity Lifecycle Framework** (see below), **Shared Sheet Write Utilities** (see below), and the other big-product items captured below and in `TODO.md → Historical backlog`.
...
Authoritative copy: `TODO.md → Future UI Standardization — Manage Pattern Rollout` (high-level mirror in `PROJECT_CONTEXT.md`). **Status:** Bills, Debts, Bank Accounts, Investments, and Houses have the Manage presentation surface; the shared tracked-editor presentation layer and Save-only Update panels passed isolated Central `@195`. Full lifecycle parity is separate: Houses, Income Sources, and Bills remain under `3o`.

Manage Bills and Manage Debts proved the presentation pattern. Bank Accounts, Investments, and Houses now share the same client-side mode/list/focus/empty-state primitives while keeping their existing writers and data rules separate. Income Sources remains the next long-lived-entity presentation candidate; transaction/append and singleton workflows should not be forced into this pattern. A Manage surface alone does not satisfy the lifecycle contract.
...
Authoritative copy: `TODO.md → Future Feature — Shared Entity Lifecycle Framework` (high-level mirror in `PROJECT_CONTEXT.md`; numbered delivery scope in `FULL_BETA_REMAINING_PLAN.md → 3o`). **Status:** active phased rollout. Debt, Bank Accounts, and Investments are complete; Houses, Income Sources, and Bills remain. **Reference implementation: the Debt lifecycle (`Active → Stop Tracking → Inactive → Reactivate`, commit `893d50d`).**

The **lifecycle** companion to Manage Pattern Rollout above. **Long-term goal:** all tracked entities eventually share the same lifecycle — `Create → Edit → Rename → Stop Tracking → Inactive → Reactivate` — across long-lived entities (Debts *(behavioral reference)*, Bank Accounts, Investments, Houses, Bills, Income Sources) so each module stops reinventing its own lifecycle UI, labels, colors, confirm dialogs, activity logging, and server guardrails. Shared pieces to extract: UI language (Active / Inactive sections, "Show inactive" toggle with count, Reactivate, Stop Tracking in Manage, common empty state + helper text; Update stays Save-only); button styles (Edit / Rename neutral, Reactivate positive/green, Stop Tracking destructive/red); confirmation copy (remains in history · excluded from live calcs · restorable from Inactive); server lifecycle helpers (deactivate / reactivate existing row, block duplicate active names, preserve history, allow-list guard preventing direct `Active` edits via generic update endpoints, activity events); activity event pattern (`<entity>_deactivate` / `<entity>_reactivate` → "Tracking stopped" / "Tracking resumed", consistent JSON details); and lifecycle diagnostics (inactive-with-balance, duplicate active names, inconsistent `Active` values). A separate Danger Zone panel is intentionally not part of the shared pattern. Phased: document Debt behavior → inventory other modules → extract CSS/copy → extract server helpers → migrate one module at a time with runtime validation. **Priority:** resume **Houses → Income Sources → Bills** after the core Rolling Financial Plan increment and close `3o` before broader external beta.
