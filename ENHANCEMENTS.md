...
Authoritative roadmap lives in `TODO.md → Product Maturity Stages` (Stage 1–6 + `Current Engineering Priorities` + `Beta Gate`; high-level mirror in `PROJECT_CONTEXT.md`). **Current stage: Stage 3 — Beta Readiness** (Stages 1–2 complete; Family Beta ~97–98% · External Beta ~92%; remaining work is the Beta Gate — Validation Agent, Financial Integrity convergence, remaining recovery adoption paths — not major feature development). Mirror here is short on purpose:

- **Active now (Stage 3 — Beta Gate):** (1) **Validation Agent** — automated regression-detection release gate (highest engineering priority); (2) **Financial Integrity convergence** — canonical basis + Planner/Dashboard/Rolling convergence to $0.01 + Asset/Planner/Dashboard audit modules (foundation shipped); (3) **Golden Workbook Convergence** — newly provisioned workbook converges toward the **Golden Workbook** / visual source of truth until indistinguishable from production (P1, L — a Family Beta quality gate and a convergence project, not a redesign); **first audit complete 2026-07-06** — ten core sheets rated by four design families — spec `GOLDEN_WORKBOOK.md`, checklist `WORKBOOK_PARITY_CHECKLIST.md`; (4) **Recovery Validation — remaining adoption paths** (6F: Auto-Adopt / Ambiguous / Name-only adoption / Orphan, then flags OFF).
