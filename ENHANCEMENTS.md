...
Authoritative copy: `TODO.md → Future UI Standardization — Manage Pattern Rollout` (high-level mirror in `PROJECT_CONTEXT.md`). **Status:** Bills, Debts, Bank Accounts, Investments, Houses, and Income have their domain-appropriate Manage presentation surface. Tracking/recovery parity is complete; House metadata Edit/Rename and Income Edit/Rename remain separate.

Manage Bills and Manage Debts proved the presentation pattern. Bank Accounts, Investments, Houses, and Income now use the same product hierarchy while keeping their existing writers and data rules separate. Transaction/append and singleton workflows should not be forced into this pattern. A Manage surface alone does not satisfy the lifecycle contract.
...
Authoritative copy: `TODO.md → Future Feature — Shared Entity Lifecycle Framework` (high-level mirror in `PROJECT_CONTEXT.md`; numbered delivery scope in `FULL_BETA_REMAINING_PLAN.md → 3o`). **Status:** tracking/recovery rollout complete across Debt, Bank Accounts, Investments, Bills, Houses, and Income. Common server-helper extraction remains deferred; House metadata Edit/Rename and Income Edit/Rename are separate gaps. **Reference implementation: the Debt lifecycle (`Active → Stop Tracking → Inactive → Reactivate`, commit `893d50d`).**

The **lifecycle** companion to Manage Pattern Rollout above. The shipped tracking/recovery contract is `Active → Stop Tracking → counted Inactive discovery → Reactivate the same preserved identity`; Add never substitutes for recovery, customer confirmation is CashCompass-owned, and zero/unknown/error does not show a false inactive affordance. Shared server lifecycle helpers and diagnostics may still be extracted later, but that refactor is not required to call the behavior complete. Edit/Rename remains feature-specific and must be scoped separately where missing.
