...
- **Bill maintenance, Donation management, and AutoPay formatting ready for commit (2026-08-03).** Bill edits now coordinate a payee rename with the exact linked current-year Cash Flow Expense row under lock, reject collisions or ambiguous links, verify both writes, require immutable audit history, roll back every companion change on failure, preserve the verified category across the first-load race, and auto-fit only the affected Payee columns after durable success. Donations now retain the two-column Add view with a blue read-only Previous donations panel and provide a separate Manage donations view for full stable-row edits, including guarded same-year updates and existing-block tax-year moves with audit and rollback. Monthly and expanded AutoPay writes finish with canonical red negative-currency formatting. Permanent focused local and disposable-harness regressions cover all four fixes; the full local `npm test` gate and `git diff --check` pass. The user manually confirmed the isolated Central `@304` Bill rename/category behavior, Donation experience, and AutoPay display. Beta remains `@106`; the bounded deployment/workbook, formulas, administrator identity, and workbook schemas were untouched. Rollback is to revert the eventual commit and, for isolated Central, redeploy `@300`; successful user workbook edits remain preserved in immutable history and are not automatically reversed.
