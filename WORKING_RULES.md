...
- **Bounded project and workbook hard stop.** Codex must never run `clasp push`, `clasp deploy`, `clasp undeploy`, or any other source/deployment mutation against the bounded Apps Script project; this restriction is permanent and is not lifted by general push/deployment approval. The owner alone performs every bounded push, bounded deployment, and bounded visual check. Codex must also never run a Harness writer against the bounded or mapped-user workbook or use that workbook for automated mutation testing. Automated writers may target only their own continuously re-verified marked disposable workbook; isolated Central deployment still requires explicit approval.
