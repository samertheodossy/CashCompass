...
- **Bounded-workbook hard stop.** Codex must never deploy to the bounded Apps Script project, run a Harness writer against the bounded or mapped-user workbook, or use that workbook for automated mutation testing. The user alone performs bounded deployment and bounded visual checks. Automated writers may target only their own continuously re-verified marked disposable workbook; isolated Central deployment still requires explicit approval.
- **Standing isolated-Central validation authorization (approved 2026-07-27).** After the active numbered item's focused and full local regressions pass, Codex may push the reviewed working-tree source to the Central Apps Script project, create a new immutable Apps Script version, advance **only** the isolated validation deployment, and run the item's guarded disposable-workbook/read-only runtime checks without asking again. This standing authorization never includes Central Beta, the bounded Apps Script project, the bounded/mapped-user workbook, Golden/configured-default workbooks, Git staging/commit/push, schema migration, or unrelated deployment changes. Report the exact isolated version and runtime evidence at the milestone checkpoint.
- **Central clasp targeting is explicit.** Every Central `clasp version`, `clasp deploy`, and `clasp deployments` command must include `--project .clasp-central.json`; never rely on the repository-default `.clasp.json`. Before an isolated deploy, inventory Central with that explicit project argument and resolve the exact isolated deployment ID. A plain `clasp` mutation command is prohibited.
