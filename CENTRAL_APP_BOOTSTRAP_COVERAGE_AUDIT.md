...
- **✅ Works — runtime-confirmed 2026-05-23.** `getRollingDebtPayoffPlan` (`rolling_debt_payoff.js:2862–2924`) has an explicit blank-workbook short-circuit: when either `INPUT - Debts` or `SYS - Accounts` is absent, it returns a neutral `{ not_set_up: true, setup_message: …, summary: { plan_status: 'NOT_SET_UP', … }, … }` envelope; the client renderer at `Dashboard_Script_RollingDebtPayoff.html:744–759` renders a calm setup-message card. Verified across three reachable states (fully blank, partial Setup / Review, both prerequisites present) by `CENTRAL_APP_BLANK_WORKBOOK_RUNTIME_REPORT.md → §4A.3.1 / §4A.4.2 / §4A.5.5`. No red banner. No sheet writes from the page-open path. No planner execution side-effect.
- _Original classification (preserved verbatim):_ "**Unknown / likely partial.** Reads INPUT - Debts and other planner inputs. Behavior on a blank workbook needs a runtime test; the strict read path may throw if `INPUT - Debts` is missing and the page is opened directly. Note: `INPUT - Debts` is the bootstrap registry's responsibility, so it can be created from a Setup / Review pass before Rolling Debt Payoff is opened." — superseded by runtime evidence 2026-05-23.

### 4.16 Debt Payoff Projection (Planning page)
- **✅ Works — runtime-confirmed 2026-05-23.** `getDebtPayoffReadData` (`debt_payoff_projection.js:16–51`) has an explicit blank-workbook short-circuit: when either `INPUT - Debts` or `SYS - Accounts` is absent, it returns a zeroed envelope (`debts: []`, `summary: { totalDebtBalance: 0, … }`, empty recommendations / warnings); the page renders the zero-state via its existing renderer. The Cash Flow read at lines 69–76 is wrapped in `try` / `catch` so missing Cash Flow year sheets do not leak a banner. Verified across three reachable states by `CENTRAL_APP_BLANK_WORKBOOK_RUNTIME_REPORT.md → §4A.3.2 / §4A.4.3 / §4A.5.6`. No red banner. No sheet writes from the page-open path. No planner execution side-effect.
- _Original classification (preserved verbatim):_ "**Likely partial.** `debt_payoff_projection.js:28` reads via `getSheetNames_()` lookup but the strict read paths may throw on missing sheets. Unknown without runtime test." — superseded by runtime evidence 2026-05-23.
...
### 5.3 Rolling Debt Payoff / Debt Payoff Projection blank-workbook behavior — ✅ **resolved (was an unknown)**
- **Status (2026-05-23):** ✅ runtime-confirmed working on a blank workbook. Both modules have explicit short-circuits that return calm / zeroed envelopes when prerequisites are missing, and the client renderers handle the zero-state without leaking a red banner. Verified across the three reachable states (fully blank, partial Setup / Review, both prerequisites present) by `CENTRAL_APP_BLANK_WORKBOOK_RUNTIME_REPORT.md → §4A.3 / §4A.4 / §4A.5`. Closure record at `CENTRAL_APP_BLANK_WORKBOOK_RUNTIME_REPORT.md → §4A.12`.
- **No code change required.** The defensive guards (`!ss.getSheetByName(sheetNamesEarly.DEBTS) || !ss.getSheetByName(sheetNamesEarly.ACCOUNTS)` in `rolling_debt_payoff.js:2870–2924`; the equivalent in `debt_payoff_projection.js:28–51`; the `try` / `catch` around `readCashFlowSheetAsObjects_` in `debt_payoff_projection.js:69–76`) were already in place; the runtime test simply confirmed they fire correctly on a real blank workbook.
- _Original wording (preserved verbatim):_ "Both modules read `INPUT - Debts`. After the bootstrap registry creates `INPUT - Debts` via `ensureOnboardingDebtsSheetFromDashboard`, the page may render an empty calm state — but **this has not been verified end-to-end on a fully blank workbook.** Adding it to the runtime test matrix (§8) is the cheapest path to certainty." — the runtime test predicted in the original wording has now been executed; the predicted "empty calm state" is what was observed.
...
- ~~Runtime confirmation of Rolling Debt Payoff / Debt Payoff Projection on a fully blank workbook — gap §5.3.~~ ✅ **Done 2026-05-23.** Both pages runtime-confirmed working across three reachable blank-workbook states per `CENTRAL_APP_BLANK_WORKBOOK_RUNTIME_REPORT.md → §4A.12`. No code change required; the defensive guards were already in place.
...
- **`debt_payoff_projection.js`, `rolling_debt_payoff.js`** — read INPUT - Debts via `getSheetNames_()` lookup. ✅ **Confirmed defensive — runtime-confirmed 2026-05-23.** Both modules have explicit blank-workbook short-circuits and return calm / zeroed envelopes when prerequisites are missing. See §5.3. _Original wording preserved verbatim:_ "unknown whether strict throw or defensive. Needs runtime verification." — verification now done; result is "defensive."
...
The audit closes the question of whether the existing ensure-\* coverage is sufficient. Findings (updated after the §5.6 fix shipped in commit `4e6af6d`, after the 2026-05-22 runtime retest reclassified §5.2, and after the 2026-05-23 runtime check resolved §5.3):

- **One confirmed gap remains** for hands-off zero-sheet onboarding: `INPUT - Donation` (§5.1). Additive ensure-\* helper opportunity that mirrors the existing pattern. **This is now the only remaining true onboarding blocker on the family beta first-run path.**
...
- **One previously-unknown row is now resolved:** Rolling Debt Payoff / Debt Payoff Projection blank-workbook behavior (§5.3). Runtime-confirmed working across three reachable blank-workbook states on 2026-05-23 per `CENTRAL_APP_BLANK_WORKBOOK_RUNTIME_REPORT.md → §4A.12`. Both pages have explicit short-circuits and calm zero-state renderers; no red banner, no sheet writes, no planner execution side-effect. No code change required.
- **No runtime unknowns remain** on the family beta first-run path. The blank-workbook runtime matrix is effectively complete.
...
Given that picture (Pass 1 runtime test matrix now effectively complete after the 2026-05-23 closure of §5.3), Pass 2's scope is narrow:
...
- **Blank-workbook readiness today:** strong end-to-end on every surface in scope for the family beta first-run path. After commit `4e6af6d` (2026-05-22) closed the §5.6 House Values regression, the 2026-05-22 retest reclassified §5.2 (`HOUSES - <Name>` is auto-created), and the 2026-05-23 runtime closure resolved §5.3 (Rolling Debt Payoff / Debt Payoff Projection runtime-confirmed working), the blank-workbook runtime matrix is **effectively complete**. Most major surfaces work or lazy-bootstrap calmly; the six canonical core sheets are covered by `sheet_bootstrap.js`'s registry. Dashboard Overview, Setup / Review, Quick Add, Cash Flow, Bills (read), Debts (read), Bank Accounts, Investments, Property Performance, Retirement, Upcoming Expenses, Activity Log, House Values (Create house), House Expenses (add expense for a house created via the normal flow), Rolling Debt Payoff, and Debt Payoff Projection all handle blank state correctly. **One confirmed gap remains: §5.1 Donations.**
- **Confirmed gap (still open):** one — (§5.1) `INPUT - Donation` no ensure helper. This is now the only remaining true onboarding blocker on the family beta first-run path.
...
- **Previously-unknown row (now resolved):** (§5.3) Rolling Debt Payoff / Debt Payoff Projection blank-workbook behavior — ✅ runtime-confirmed working 2026-05-23 per `CENTRAL_APP_BLANK_WORKBOOK_RUNTIME_REPORT.md → §4A.12`. Both pages have explicit short-circuits and calm zero-state renderers; verified across three reachable blank-workbook states (fully blank, partial Setup / Review, both prerequisites present). No code change required.
- **Unknown:** none. The blank-workbook runtime matrix has no remaining unknowns on the family beta first-run path.
...
- **Static-analysis misses surfaced by runtime tests (historical record):** the audit's §4.9 originally classified House Values onboarding as "Lazy-bootstraps." The runtime test in `CENTRAL_APP_BLANK_WORKBOOK_RUNTIME_REPORT.md → §4.9.2` falsified that classification. Root cause traced to a pre-validation read path the static trace did not follow (§5.6, now closed). The 2026-05-22 retest after that fix also revealed a second static-analysis miss: §5.2 had been classified as "no canonical creator," but the creator exists as `createHousesExpenseSheet_` in `house_values.js` and is invoked from `addHouseFromDashboard` step 4 — the static trace stopped at `house_expenses.js` (which does not lazy-create) and missed the upstream call. The 2026-05-23 closure of §5.3 produced the opposite kind of evidence: the static prediction ("may render an empty calm state") was *correct*, but it required a runtime test to convert that prediction to confirmed behavior. All three lessons are exactly what the runtime test was designed to surface — static analysis catches structural gaps and produces hypotheses; runtime test catches ordering, execution-path, and cross-module integration gaps, and confirms or falsifies the hypotheses. The §5.6 fix's static recheck also surfaced the same latent pre-validation-before-ensure shape in Debts and Investments (no current regression, queued as defense-in-depth).
- **Recommended next step:** Pass 2 — one narrow additive ensure-\* prompt for the remaining confirmed gap (§5.1 Donations). With §5.3 now resolved by runtime evidence and no other unknowns remaining on the family beta first-run path, Donations is the cheapest, smallest next step before central-mode planning begins. The optional defense-in-depth reorders (Debts / Investments), the optional defense-in-depth lazy-create (House Expenses for legacy workbooks), and the cosmetic House Values polish can be queued after, individually, and are not required for the family beta first-run path.
