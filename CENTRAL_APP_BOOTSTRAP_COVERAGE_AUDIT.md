...
- **Does not bootstrap per-house sheets.** A `HOUSES - <Name>` sheet has **no automatic creator**. `addHouseExpense` throws `'House sheet not found: ' + payload.house` (`house_expenses.js:49`). Separate gap §5.2 — still open.
...
The audit closes the question of whether the existing ensure-\* coverage is sufficient. Findings (updated after the §5.6 fix shipped in commit `4e6af6d`):

- **Two confirmed gaps remain** for hands-off zero-sheet onboarding: `INPUT - Donation` (§5.1) and `HOUSES - <Name>` per-house tabs (§5.2). Both are additive ensure-\* helper opportunities that mirror the existing pattern.
...
- **Blank-workbook readiness today:** strong on the Houses write path after commit `4e6af6d` (2026-05-22) closed the §5.6 regression. Most major surfaces work or lazy-bootstrap calmly; the six canonical core sheets are covered by `sheet_bootstrap.js`'s registry. Dashboard Overview, Setup / Review, Quick Add, Cash Flow, Bills (read), Debts (read), Bank Accounts, Investments, Property Performance, Retirement, Upcoming Expenses, Activity Log, and now House Values (Create house) all handle blank state correctly. Two confirmed gaps remain (§5.1 Donations, §5.2 per-house `HOUSES - <Name>` tabs).
- **Confirmed gaps (still open):** two — (§5.1) `INPUT - Donation` no ensure helper, (§5.2) `HOUSES - <Name>` per-house tabs no canonical creator.
...
- **Optional / not blocking:** (a) schema-version marker (§5.5); (b) cosmetic polish of generated `INPUT - House Values` / `SYS - House Assets` sheet formatting on first creation (runtime-report §6 row 4); (c) defense-in-depth reorder of the ensure block in `addDebtFromDashboard` and `addInvestmentAccountFromDashboard` to mirror the Houses fix (runtime-report §6 row 5; latent shape, no current runtime regression).
- **Static-analysis miss surfaced by runtime test (historical record):** the audit's §4.9 originally classified House Values onboarding as "Lazy-bootstraps." The runtime test in `CENTRAL_APP_BLANK_WORKBOOK_RUNTIME_REPORT.md → §4.9.2` falsified that classification. Root cause traced to a pre-validation read path the static trace did not follow (§5.6, now closed). This is exactly the value the runtime test was designed to produce — static analysis catches structural gaps, runtime catches ordering and execution-path gaps. The §5.6 fix's static recheck also surfaced the same latent shape in Debts and Investments (no current regression, queued as defense-in-depth).
- **Recommended next step:** Pass 2 — narrow additive ensure-\* prompts for the two remaining confirmed gaps (§5.1 Donations, §5.2 per-house tabs). Each is a separate Cursor implementation prompt requiring explicit user approval. The optional Debts / Investments defense-in-depth reorders and the cosmetic House Values polish can be queued after, individually.
- **Family beta proof readiness:** after Pass 2 closes the two remaining confirmed gaps (§5.1, §5.2), the additive ensure-\* chain is sufficient for a hands-off first-run flow against a user-created workbook. The remaining layers needed for the family beta proof (Drive API workbook creation, per-user mapping store, central deployment posture, first-run UX, recovery surfaces, rollback procedure) are documented in `CENTRAL_APP_MINIMAL_BETA_PROOF.md → §6` and are independent of this audit's conclusions.
