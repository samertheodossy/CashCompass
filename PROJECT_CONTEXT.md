...
- **Stage:** **Stage 3 — Beta Readiness (current).** **Stage 1 (Core Platform), Stage 2 (Product Hardening), P0 Project Stabilization, House Financial Accuracy V1, and Financial Integrity Phase 3 are complete**, and **Recurrence Engine V2 (Weekday & Biweekly scheduling) shipped 2026-07-09.** The capability estimate remains **Family Beta Readiness ~97–98% · External / Public Beta Readiness ~92%**. The last formal independent advocate score is **8.3/10** on isolated Central `@180`; it remains official because the isolated `@196` **7.2/10** run was read-only and evidence-limited, not a comparable writer-inclusive rescore. Isolated Central has advanced through `@319`: source `8aa4bf5bf58259598289d79368eea944e015d43e` / `@318` completed the exact-owner Performance Planner gate with 20/20 pairs, 40 executions, zero failed samples, Restricted sharing and verified cleanup, and combined p50/p95 **12.600/16.773 s** against **30/60 s** budgets. Diagnostic `@319` then completed cross-flow measurement `4d`: Save **1 ms acknowledgement / 8.332 s completion**, loaded navigation **17/18 ms p50/p95**, and representative populated Overview **11.776 s**, with suite PASS, Restricted sharing, and verified Trash cleanup. Save completion exceeded the directional 6 s target and remains a `4f` follow-up. The disposable runner is OFF. `BETA_10_OUT_OF_10_PLAN.md` defines the ≥95/100 release standard and monetization foundations; `FULL_BETA_REMAINING_PLAN.md` is the authoritative numbered open inventory with estimates, regression homes, and the optimized execution model.
...
| Current | Remaining UX/accessibility beginning with `3e` | Complete `3e`–`3j` and `3m`; reserve `3k` for the exact frozen candidate |
| Next | Deferred linked Bill, Upcoming, and House Expense correction; entity/audit disposition | `5j`–`5m` pass fail-closed client/server and guarded disposable-workbook evidence |
| Candidate preparation | Ordinary Save performance follow-up, final financial/workbook/recovery proof, operations/support, and monetization-ready architecture | Complete remaining `4f`, `5a`–`5f`, `6a`–`6f`, and `7a`–`7f` |
...
- **✅ Milestone complete — Validator Phase 2, scenario packs, Release Readiness controls, Planner percentile gate, and `4d` cross-flow measurement.** The July `@141` run proved Provisioning, Workbook Drift, Schema Evolution V1, the formal schema registry, Formula / Conditional-Formatting / Named-Range validators, aggregate Workbook Health, the required chunked server inventory, First-Run UX E2E, Populated Dashboard E2E, Recovery Live, and resumable Release Readiness controls. The 2026-08-05 exact-owner `@318` campaign ratified Financial Plan refresh p50/p95 budgets; diagnostic `@319` completed Save, loaded-navigation, and populated-Overview measurement with PASS/Restricted/cleanup proof. The admin **Validation & Testing console** (`ValidationTestingUI.html` + `validation_testing_server.js`, route `?view=validation`) is the consolidated Validator, suite-inventory, evidence, launch, progress, and verdict surface. Still open before broad Beta: ordinary Save completion follow-up under `4f`, conditional Planner replay if the frozen candidate differs, and the complete exact-candidate archived verdict. See `VALIDATOR_ARCHITECTURE.md`, `TEST_HARNESS_ARCHITECTURE.md`, `VALIDATION_TESTING_CONSOLE.md`, and `TESTING_URLS.md`.
...
- House expenses
- Bank accounts — **Update** / **Add new** / **Manage accounts**; Manage lists active accounts, routes the existing guarded Update / Stop tracking actions, and owns Import review / CSV as secondary maintenance tools. Soft deactivate via `deactivateBankAccountFromDashboard` in `bank_accounts.js` sets **Active = No** on every **INPUT - Bank Accounts** row for the account across all year blocks and on the mirror **SYS - Accounts** row; preserves month history, totals / delta, **Current Balance**, **Available Now**, **Min Buffer**, **Type**, **Use Policy**, **Priority**, and reserves the name. Existing Add behavior, formatting inheritance, activity logging, first-run sheet safeguards, and active filtering remain unchanged.
