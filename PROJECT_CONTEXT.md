...
- **Current focus — P0 Project Stabilization (2026-07-20):** the known P0 ProductDecision inventory is resolved (Settings typography ratified 16pt header / 14pt body), and the static Tier-2 Central resolver sweep is closed: no user-facing production path directly acquires the active spreadsheet outside `getUserSpreadsheet_()`. Remaining literal calls are intentional bound/dev/safety utilities. **The remaining P0 Central gate is live Recovery Validation 6F on a named disposable account** (confirmed-zero, MEDIUM OFF/ON, ambiguity, failure, stale, and cross-user isolation; flags OFF afterward), plus the broader beta-readiness evidence review. Bills Due → Pay is committed (`fdf4a30`) with natural runtime evidence pending.
...
- Recovery Validation 6F disposable-account runtime matrix remains the P0 Central evidence gate
