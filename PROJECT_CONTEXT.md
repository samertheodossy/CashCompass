...
- **Stage:** **Stage 3 — Beta Readiness (current).** **Stage 1 (Core Platform) and Stage 2 (Product Hardening) are complete**, and **Recurrence Engine V2 (Weekday & Biweekly scheduling) shipped 2026-07-09.** Maturity (estimated): **Family Beta Readiness ~97–98% · External / Public Beta Readiness ~92%**. The remaining pre-beta work is the **Beta Gate**, sequenced by the **confirmed execution order (2026-07-09) — (1) Bills Due Pay runtime validation, (2) Investments cleanup, (3) Recovery completion, (4) Validation Agent, (5) Golden Workbook polish, (6) Family Beta polish** (Financial Integrity Phase 3 is a Beta-Gate requirement sequenced alongside 3–5); not major feature development. Authoritative stage roadmap (Stage 1–6) + Current Engineering Priorities + Beta Gate live in `TODO.md → Product Maturity Stages`; a high-level mirror is in `## Product Maturity Stages (high-level)` below.
...
- **Current focus — Stage 3 Beta Readiness (confirmed execution order, 2026-07-09):** (1) **Bills Due Pay runtime validation** — confirm the shipped Pay occurrence bridge on the next genuine paid bill, then commit; (2) **Investments cleanup** — finish in-flight Investments convergence + runtime-validate + commit `investments.js` WIP; (3) **Recovery completion — remaining adoption paths** — ratified Recovery Architecture (unconditional candidate detection + Auto-Adopt + Ambiguous + Name-only adoption + Orphan + provision-after-recovery duplicate guard on a disposable account, then flags OFF); (4) **Validation Agent** — the automated release gate that protects the reconciled numbers; (5) **Golden Workbook polish** — converge fresh provisioning toward the Golden/Canonical Workbook (visual source of truth); first audit complete, **Validator-driven engineering convergence complete for the Operational / Financial Ledger / SYS / Special families (incl. Cash Flow, 2026-07-12)**; convergence passes remaining for Investments / House Values / Retirement + the unaudited sheets; (6) **Family Beta polish** — Workbook / UX polish + Beta Gate assembly + go/no-go. **Financial Integrity — Phase 3 (convergence)** (canonical basis + Planner/Dashboard/Rolling convergence to $0.01 + Asset/Planner/Dashboard audit modules — *the numbers must reconcile*) is a Beta-Gate requirement sequenced alongside (3)–(5). Remaining recovery slices (Stage 4): 6D.2b Create New Workbook, 6E.2 Admin Set Mapping.
- **Status snapshot (2026-07-09):** Central Architecture ~95%+; Recovery Architecture ~90% implemented (ratified design in `CENTRAL_APP_RECOVERY_COMPLETION_PLAN.md`; **destructive/admin paths validated**, only Auto-Adopt / Ambiguous / Name-only adoption / Orphan remaining); **Financial Integrity foundation shipped** (Audit Framework + Debt Audit + shared Active helper + `NOT_INITIALIZED`; Phase 3 convergence remaining); **Recurrence Engine V2 (Weekday & Biweekly scheduling) shipped + runtime-validated**; **Bills Due → Pay occurrence bridge complete** (natural runtime validation pending); **Golden Workbook first audit complete (2026-07-06)**; **Validator (read-only Golden Workbook parity) Phase 1 complete** and **Validator-driven engineering convergence complete for the Operational / Financial Ledger / SYS / Special families (2026-07-12)** — remaining diffs are intentional (KeepCentral / ProductDecision / IgnoreNoise); still open: audit + convergence of the remaining sheets (HOUSES, OUT) and the numbers-reconciliation release gate (Financial Integrity Phase 3). See `VALIDATOR_ARCHITECTURE.md`, `GOLDEN_WORKBOOK.md`, `WORKBOOK_PARITY_CHECKLIST.md`. Family Beta Readiness ~97–98%; External Beta Readiness ~92%.
...
- **✅ Milestone complete — Validator-driven Golden/Canonical convergence (2026-07-12).** Completed and closed:
  ...
  - **Central migration (Phase 1)** — user-facing modules resolve the correct workbook via `getUserSpreadsheet_()` (no `getActiveSpreadsheet()` null bugs on the audited paths). *(Tier-2 sweep of non-critical/dev paths continues — see Domain Completion Matrix.)*
...
- **✅ Milestone complete — Validator Phase 2 foundation + Test Harness Foundation V1 (2026-07-13).** Shipped: **Provisioning** (structural gate — `validator_provisioning.js` / `validatorRunProvisioning()`), **Workbook Drift** (advisory — `validator_drift.js`), and **Schema Evolution V1** (advisory, version-aware Workbook Type + Compatibility — `validator_schema.js`); the **Test Harness Foundation V1** (`test_harness_*.js`: fail-closed guard + disposable-workbook lifecycle + `assertDisposableTarget_` + one `SMOKE-PROVISION-DONATION` scenario + report; editor runners `testRunSmoke()` / `testRunSmokeTrash()`); and the admin **Validation & Testing console** (`ValidationTestingUI.html` + `validation_testing_server.js`, route `?view=validation`) that runs those Validator checks and the smoke scenario (Keep/Trash) from the browser. Still open in Phase 2: Formula / Conditional-Formatting / Named-Range validation, the aggregate Workbook Health report, scenario packs (REGRESSION/RECOVERY/STRESS), and the Release Readiness gate. See `VALIDATOR_ARCHITECTURE.md`, `TEST_HARNESS_ARCHITECTURE.md`, `VALIDATION_TESTING_CONSOLE.md`.
...
- **Next milestones (see `ROADMAP.md` — the authoritative P0–P4 priority stack):** immediate priority is **Project stabilization** (P0 — documentation, remaining ProductDecision items, beta readiness, Central verification); the next **engineering** milestone is the remainder of **Validator Phase 2** (P1 — provisioning / drift / schema validation **and the Test Harness foundation + Validation & Testing console are already shipped**; still ahead: conditional-format / formula / named-range validation + the aggregate Workbook Health report + scenario packs + the release-readiness gate, `VALIDATOR_ARCHITECTURE.md`); the next **product-model** milestone is **House Financial Accuracy** (P2 — rental-property cash-flow accuracy incl. financing costs, one shared house cash-flow model, `ROADMAP.md → House Financial Accuracy`).
...
Stable and family-beta capable; provisioning proven across multiple accounts; the recovery destructive/admin paths are validated; core + lifecycle workflows + Bills Due performance + Recurrence Engine V2 shipped. Remaining work is the Beta Gate, not features (confirmed execution order, 2026-07-09):

- Bills Due Pay runtime validation (confirm the shipped Pay occurrence bridge, then commit)
- Investments cleanup (finish in-flight convergence + commit WIP)
- Recovery completion — remaining adoption-path validation (ratified Recovery Architecture)
- Validation Agent (automated release gate that protects the reconciled numbers)
...
- *(Beta-Gate requirement, sequenced alongside Recovery / Validation Agent: Financial Integrity — Phase 3 convergence — the numbers must reconcile.)*
...
- **Stage 3 — Beta Readiness** *(current)* — the **Beta Gate**, sequenced by the **confirmed execution order (2026-07-09)**: (1) **Bills Due Pay runtime validation** (confirm the shipped Pay occurrence bridge, then commit), (2) **Investments cleanup** (finish in-flight convergence + commit WIP), (3) **Recovery completion — remaining adoption paths** (ratified Recovery Architecture: unconditional candidate detection + Auto-Adopt / Ambiguous / Name-only / Orphan + duplicate guard), (4) **Validation Agent** (automated release gate that protects the reconciled numbers), (5) **Golden Workbook polish** (P0/P1, L — newly provisioned workbook converges toward the **Golden Workbook** / visual source of truth until visually indistinguishable from production; **first audit complete 2026-07-06** — ten core sheets rated across four design families, see `GOLDEN_WORKBOOK.md` + `WORKBOOK_PARITY_CHECKLIST.md`; a convergence project, not a redesign), (6) **Family Beta polish** (Workbook / UX polish — loading, empty-states, consistency, onboarding — + release/onboarding review). **Financial Integrity — Phase 3 (convergence)** (canonical basis + Planner/Dashboard/Rolling convergence to $0.01 + Asset/Planner/Dashboard audit modules — *the numbers must reconcile*) is a Beta-Gate requirement sequenced alongside (3)–(5). Goal: **Family Beta Release Candidate — provably correct, recoverable, and visually production-grade.**
...
**Current Engineering Priorities (confirmed execution order — 2026-07-09):** 1) **Bills Due Pay runtime validation** *(confirm the shipped Pay occurrence bridge, then commit)* · 2) **Investments cleanup** *(finish in-flight convergence + commit WIP)* · 3) **Recovery completion — remaining adoption paths** *(ratified Recovery Architecture)* · 4) **Validation Agent** *(automated release gate that protects the reconciled numbers)* · 5) **Golden Workbook polish** *(provisioning converges toward the Golden Workbook, the visual source of truth; first audit complete; convergence, not redesign)* · 6) **Family Beta polish** (Workbook / UX polish — loading, empty-states, consistency, onboarding). **Beta-Gate requirement sequenced alongside 3–5:** **Financial Integrity — Phase 3 (convergence)** *(the numbers must reconcile; foundation shipped)*. **Longer-horizon (post-Beta-Gate):** **Shared Lifecycle Framework rollout** (Debt reference → Bank Accounts → Investments → Houses → Income Sources) · **Product differentiators (V1, External Beta): Money Plan Phase 2 + Income Expected/Due Workflow**.

**Beta Gate (release-readiness target — unordered criteria, all must pass):** before broader beta every release must pass **Golden Workbook Convergence** (first audit complete; **Validator-driven engineering convergence complete for the Operational / Financial Ledger / SYS / Special families, 2026-07-12**; remaining sheets/ProductDecisions pending), **Financial Integrity** (foundation shipped; Phase 3 convergence remaining), the **Validator / Validation Agent** (read-only Golden Workbook parity **Phase 1 complete**; Phase 2 provisioning validation + the numbers-reconciliation gate still planned — see `VALIDATOR_ARCHITECTURE.md`), **Recovery Validation** (destructive/admin done; adoption paths remaining), and a **runtime regression checklist** before deployment. *(Confirmed execution order that sequences these: Bills Due Pay validation → Investments cleanup → Recovery → Validation Agent → Golden Workbook polish → Family Beta polish; FI Phase 3 alongside Recovery / Validation Agent.)*
