...
**Beta Gate (release-readiness target — unordered criteria, all must pass):** before broader beta every release must pass **Golden Workbook Convergence** (first audit complete; convergence passes remaining), **Financial Integrity** (foundation shipped; Phase 3 convergence remaining), the **Validation Agent** (not started), **Recovery Validation** (destructive/admin done; adoption paths remaining), and a **runtime regression checklist** before deployment. *(Confirmed execution order that sequences these: Bills Due Pay validation → Investments cleanup → Recovery → Validation Agent → Golden Workbook polish → Family Beta polish; FI Phase 3 alongside Recovery / Validation Agent.)*
