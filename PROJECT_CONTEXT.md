...
- **Recently completed (this initiative):**
  ...
  - **Bills Due recurrence overhaul (2026-06-12, shipped + family-beta validated):** Weekly/Biweekly bills now use **true occurrence expansion** (individual occurrences at their normal per-occurrence amount on the Due Day anchor + 7/14-day steps; the rejected monthly-burden averaging was reverted), with **per-occurrence autopay accumulation/dedupe** into the single monthly Cash Flow cell (manual-protection preserved). **Skip suppression for expanded recurrence completed** — `skipDashboardBill` now logs the `bill_skip` event regardless of cell state (the `$0` write stays blank-guarded), so a skipped Weekly/Biweekly occurrence clears and stays gone while future occurrences remain. **AutoPay + overdue visual indicators completed** — gold ⭐ star on AutoPay cards, light-red styling on overdue cards. **Weekday-aware recurrence is now shipped** — see **Recurrence Engine V2 (Weekday & Biweekly scheduling, 2026-07-09)**: true Weekly weekday + Biweekly `Anchor Date` cadence, `Weekday`/`Anchor Date`/`Schedule Effective Date` columns, prospective-only schedule changes, AutoPay `LockService` hardening. Blank scheduling fields preserve legacy Due Day **display/manual reconciliation**, but unattended Weekly AutoPay now fails closed unless a recognized Weekday exists and the occurrence lands on it.
  - **Manage Debts (2026-06-15/16; interaction convergence active 2026-08-07):** A user-friendly debt-maintenance experience under Planning → Debts (mirrors Manage Bills) so users don't hand-edit the protected `INPUT - Debts` sheet. `[Update] [Add new] [Manage Debts]` toggle; sortable debt table; one row-level **Edit** action opens a form where Account Name, terms, and balances share one **Save changes** action; **Stop tracking** reuses `deactivateDebtFromDashboard`. When Account Name changes, the server performs the coordinated, lock-guarded rename of the `INPUT - Debts` row **and** the matching `Type = Expense` Payee cell across **all** `INPUT - Cash Flow YYYY` sheets (Payee cell only; month values/formulas untouched), with stale-row protection, **duplicate-name protection** (active + inactive, case-insensitive → *"Another debt account already uses this name. Rename was not completed."*), a TOTAL-DEBT/reserved guard, Activity evidence, and best-effort rollback on partial failure. Internal coordination is not exposed as another user action. No schema changes. **Merge Debt Accounts** is documented as a separate **future** enhancement (deliberately distinct from block-on-duplicate Rename).
...
**Current implementation priority (2026-08-21):** preserve the frozen Planning foundation and begin the real customer **Cash + Credit Card Import / Refresh** accuracy milestone as separately reviewed, shadow-only slices. Within that named milestone, revolving-debt/credit-card evidence comes first because it directly controls debt-paydown decisions; bank/cash evidence follows. Normalize balances, contractual minimums, exact due dates, applicable APRs, and supporting account/statement facts with effective/observed timestamps, freshness, provenance, protected identity, and conflict state. Compare against current Planning values without silently switching authority, importing transactions broadly, or executing payments. The canonical long-term lifecycle and later order—loans/mortgages; investment accounts; holdings/tax-aware data; household domains and targeted evidence; then separately approved authority migration—live in `PART_2A_FINANCIAL_FACTS_CONTRACT.md → §§11–15`. The remaining `3o` Houses/Income implementation and Bills runtime proof, `3j` accessibility, `5j`–`5m` corrections, operations/privacy, rewards, bill-gap modeling, broader portfolio optimization, and retirement/property optimization remain deferred; none reopens frozen Overview/Debt UX.
...
Central `@356` (run `20260815-115249-7301`, 7/7 assertions, Provisioning/Drift
PASS, verified Trash cleanup). Part 2A-2 passed isolated Central `@357`: two
registered scenarios passed 25/25 assertions in 18.8 seconds, Provisioning and
Drift passed, both marker-verified fixtures were verified TRASHED, and the
disposable runner returned OFF. The implementation adds lazy, append-only
`SYS - Financial Facts`, all 23 typed fact contracts, exact Effective-As-Of vs
Observed-At provenance, centralized `DATA_QUALITY_POLICY_V1`, policy-owned
current selection, decision-specific confidence/actionability, idempotent
evidence, and an in-memory rebuildable current projection. Existing domain
...
**Part 2A-4 runtime-proven candidate (2026-08-17):** `debt_import.js` adds the
source-neutral `DEBT_EVIDENCE_V1` boundary with OFX/QFX, structured snapshot,
and verified-manual adapters. It preserves balance, minimum, next-payment,
credit, and separately typed APR evidence behind deterministic Preview/Apply.
Planning APR is selected only when source semantics prove applicability to the
carried balance; ambiguity remains Review Required. Balance, interest-ranking,
payment-obligation, and exact-payoff readiness remain distinct. Current zero
balances do not release minimums, `INPUT - Debts` remains authoritative, and
Planning is unchanged. Isolated Central `@363` suite run
`20260817-074631-0fb6` passed 1/1 scenario and 33/33 assertions in 44.7 seconds
with restricted sharing, Provisioning, Drift, and verified Trash cleanup all
...
**RFP-3b completed checkpoint (implemented 2026-08-14; frozen 2026-08-21):** the same module now adds a pure
...
Final isolated Central `@387`, owner visual acceptance, and the accepted
30/90-day safety proof close this foundation in commit `9a6cb96`.

**RFP-3 Capital Deployment Pace correction (accepted/frozen):** the read-only
...
Debt UX are therefore **ACCEPTED / STABLE CHECKPOINT**. The deterministic
financial safety certification is now **PASS / COMPLETE** after the 2026-08-21
post-decision 30/90-day proof. The
...
The post-decision proof now emits one read-only
`RFP_3_POST_DECISION_SAFETY_V1` packet from the canonical plan and fails closed
unless account sources reconcile, buffers and exclusions remain intact, current
requirements and gross 30/90-day obligations are covered by current cash,
reserve and preferred liquidity survive, staged capital reconciles, pacing
authority exists, source ownership is unique, recurrence inputs are resolved,
payment evidence is audited/deduplicated, and recommendation remains distinct
from confirmation. Isolated Central `@387` passed 124/124 assertions. A
follow-up Bills regression on the same source passed 123/123 assertions and
proved that immutable `bill_add` evidence supplies the creation-month floor
when a legacy row's Schedule Effective Date is blank. The
following milestone is real customer
...
The final bounded 90-day reconciliation also passed. Gross operating
obligations are `$125,454.92`; the protected reserve is the separately computed
net floor. The reserve permits at most `$83,636.61` of forecast-income offset
while preserving one third of gross outflows, so `$125,454.92 - $83,636.61 =
$41,818.31`, exactly the displayed protected amount. Property contingency is
already inside gross obligations; account buffers and Do Not Touch cash remain
separate, and no additional below-line reserve component applies. Current cash
after the recommendation is `$133,010.14`, leaving `$7,555.22` above the full
unreduced gross obligations (`1.06x`). Because optional deployment is capped
against that gross current-cash test, unreceived future income is not required
for 90-day solvency.

**Frozen Planning foundation handoff (2026-08-21):** Overview is **FROZEN**,
Debt is **FROZEN**, 30/90-day post-decision safety is **PROVEN**, and the
Planning foundation is a **STABLE CHECKPOINT**. The checkpoint certifies the
calculation semantics and safety lifecycle; it does not certify that manually
maintained balances, minimums, due dates, or APRs are institution-current.
Those facts are known to be stale. The next accuracy milestone is shadow-only
**Cash + Credit Card Import / Refresh**: ingest authoritative evidence with
effective date, observation time, freshness, and provenance; compare it with
existing Planning values; make no silent authority switch; execute no payment;
and avoid broad transaction import unless a separately approved slice includes
it. `ROLLING_FINANCIAL_PLAN_DECISION_CONTRACT.md` contains the complete frozen
invariant and next-phase boundary.

The import authority lifecycle is canonical in
`PART_2A_FINANCIAL_FACTS_CONTRACT.md → §§11–15`: evidence is normalized and
matched to stable identity, compared with existing Planning values, shown for
review, and only later promoted at an explicit authority checkpoint before
recalculation. Equivalent facts must produce the same financial semantics
...
`/Users/stheodos/Desktop/Samer-Financial-Planner`, branch `main`, based on
`8fc333b0c41b68777cff2d734a71db2938432f88` and tracking `origin/main`. The
owner approved the combined Overview/Debt financial freeze, 30/90-day proof,
and new-bill recurrence fix for commit/push on 2026-08-21. No
`.codex-stage-*` or generated material is part of the candidate.
...
**Completed `3b` evidence (2026-07-31):** shared empty, unavailable, retry, and failure presentation now covers the core Bank, Investment, Debt, Purchase, Upcoming, and Bills surfaces. Fresh Bank, Investment, and Debt editors open directly in **Add new**; Purchase results remain hidden until a successful simulation; empty Upcoming hides Dismiss guidance; and a truly empty Bills workspace opens **Add bill**. Malformed and failed loads do not masquerade as valid empty responses, active Bills still remain on **Due this period**, and manual retry clears stale failure state after success. The full local suite passes. Guarded First-Run run `FR-e92641a0-2c59-45da-a90c-8b8780410f76` passed 14/14 on isolated Central `@296`, including the exact guided-Add and Purchase-guidance assertions, Restricted single-owner sharing, zero browser errors, and verified Trash cleanup. The final contextual refinements were user-verified on a fresh workbook at isolated Central `@298` and have permanent local coverage. Source commit `3e2db4f` is on `origin/main`. Beta remains `@106`; no bounded deployment, bounded-workbook write, schema, formula, or financial rule changed.

**Completed `3c` evidence (2026-07-31):** the full local regression suite passes, and read-only isolated Central `@293` browser validation confirms the Debt accounts maintenance source, read-only Debt Overview, actionable Rolling Debt Payoff, exact cross-links, corrected Help language, and the generic Activity **Date** header. `@293` replaces the inline cross-links with a consistent, responsive **Related views** secondary-button row on all three Debt surfaces; the user completed the visual check and confirmed that this presentation is much better. Source commit `7c0e2ac` is on `origin/main`. Beta remains `@106`; no bounded deployment, bounded-workbook write, workbook schema, formula, or financial rule changed.

**Completed `3a` evidence (2026-07-31):** contextual loading states now cover Overview, Planning, tracked-entity pickers, onboarding, Properties, and Admin Diagnostics. Async selectors and dependent actions fail closed until valid selections exist, paired Overview placeholders announce once, and stale Bank responses cannot overwrite newer navigation state. The full local suite passes; read-only isolated Central `@294` validation confirmed the loading, settled-empty, rapid-navigation, and 390 px behavior with zero browser warnings/errors, and the user approved the visual result. Source commit `be76bdb` is on `origin/main`. Beta remains `@106`; no bounded deployment, bounded-workbook write, schema, formula, financial rule, or production writer changed.

### Beta delivery forecast — sequence reconciled 2026-07-31; calendar pending rebaseline
...
- **Controlled Family Beta:** available now for the existing small, supervised
  ...
  deployment.
- **Broad invited Beta decision:** no current committed date. The former
  **2026-09-24** target and **2026-10-02** contingency boundary require
  rebaselining after the deferred `5j` redesign is measured.
- **Current schedule signal:** the execution sequence is clear, but the calendar
  is not reliable enough to publish. Remaining UX/accessibility and compound
  Activity correction are the largest implementation uncertainties.
...
|---|---|---|
| Completed through Jul 27 | HTTP 0 recovery, exact-candidate ownership, core Bills Pay, tracked-editor convergence, Activity action truth, Overview trust, unattended orchestration, controlled loading, Skip/Stop safety, and Retirement runtime | Permanent regressions plus isolated evidence through `@211`; Wave 1 complete, Restricted fixtures verified in Trash, and bounded product spot-check remained user-controlled |
| Completed early Jul 28–29 | Activity correction foundation (`5h`) and direct Cash Flow/Donation correction (`5i`) | Durable operation identity, preview, exact-state preconditions, immutable correction evidence, clean isolated `@240` validation, and commits `d040a34` / `16b6a8f` complete |
| Completed through Jul 31 | UX `3c`, `3a`, and `3b` | Commits `7c0e2ac`, `be76bdb`, and `3e2db4f`; isolated Central through `@298`; Beta `@106`; bounded user-controlled |
| Completed Aug 1 | Bill Skip presentation and Bills Due source authority | Commit `433bfed`; `REG-041`/`REG-042`; user-verified isolated Central `@299`; Beta and bounded unchanged |
...
| Current | Frozen Planning foundation, accuracy milestone, then pre-publish product consistency/shared lifecycle | Resume `3o` one guarded module at a time; Debt, Bank, and Investments are complete through isolated `@343`, Bills is source-complete pending runtime proof, and Houses/Income remain |
...
failed gate controls the decision. The former September 24 target and October 2
contingency boundary require rebaselining after the deferred `5j` redesign is
measured.
...
- **✅ Milestone complete — Validator Phase 2, scenario packs, Release Readiness controls, Planner percentile gate, `4d` cross-flow measurement, and ordinary Save `4f` investigation.** The July `@141` run proved Provisioning, Workbook Drift, Schema Evolution V1, the formal schema registry, Formula / Conditional-Formatting / Named-Range validators, aggregate Workbook Health, the required chunked server inventory, First-Run UX E2E, Populated Dashboard E2E, Recovery Live, and resumable Release Readiness controls. The 2026-08-05 exact-owner `@318` campaign ratified Financial Plan refresh p50/p95 budgets; diagnostic `@319` completed cross-flow measurement; and `@322` reduced ordinary Save p50/p95 to 6.520/9.418 s with PASS/Restricted/cleanup proof while retaining the strict 6 s near-miss. The admin **Validation & Testing console** (`ValidationTestingUI.html` + `validation_testing_server.js`, route `?view=validation`) is the consolidated Validator, suite-inventory, evidence, launch, progress, and verdict surface. Still open before broad Beta: conditional Planner replay if the frozen candidate differs and the complete exact-candidate archived verdict. See `VALIDATOR_ARCHITECTURE.md`, `TEST_HARNESS_ARCHITECTURE.md`, `VALIDATION_TESTING_CONSOLE.md`, and `TESTING_URLS.md`.
...
- **Next milestones:** Financial Integrity Phase 3 is complete: isolated Validation `@156` suite `20260722-164849-6081` passed all 53 canonical-position, shared-consumer, audit, History, and release-inventory assertions plus every safety gate and verified Trash. The exact-owner Planner campaign is complete for `8aa4bf5` / isolated `@318`, with its 30/60 s p50/p95 budgets ratified, and `4d` cross-flow measurement is complete on diagnostic `@319`. Before broad Beta, resolve the ordinary Save completion signal under `4f`, complete the other candidate-preparation gates, freeze the candidate, replay Planner percentiles if source/deployment differs, run the complete exact-owner Release Readiness inventory, complete a supervised cohort observation window, and run the 10/10 scorecard. Broad or monetized beta waits for the exact candidate to pass `BETA_10_OUT_OF_10_PLAN.md`.
...
Read-only, admin-gated **Audit Framework** + **Debt Audit** + **shared debt Active helper (Phase 2)** + the **`NOT_INITIALIZED`** state are shipped. Canonical-basis convergence and its isolated evidence are complete; final exact-release-candidate replay remains a separate release gate.
...
- Validator Phase 2, Test Harness Foundation V1, required non-performance scenario packs, and bounded Release Readiness controls are runtime-proven; the Planner percentile gate is ratified for `8aa4bf5` / `@318`, while the complete exact-candidate verdict remains open
...
Architecture, recovery, core/lifecycle workflows, the frozen Planning foundation, and the automated Release Readiness path are built. Ordinary Save `4f` and financial-model convergence are complete; remaining gaps include final-candidate replay if source/deployment changes, regression prevention, user-lifecycle handling, and operational support.

- Conditional frozen-candidate percentile replay + final Release Readiness verdict + regression prevention
...
## Account facts now; broad Account Aggregation & Transaction Import later

The current shadow-only Cash + Credit Card Import / Refresh accuracy milestone
is the first account/statement-fact layer of this direction. Full connectivity,
broad transaction ingestion, categorization, spending analysis, and an
aggregation platform remain later capabilities. The canonical fact lifecycle
and implementation order live in
`PART_2A_FINANCIAL_FACTS_CONTRACT.md → §§11–15`; this section preserves the
larger platform vision without creating a separate authority model.
...
**Status:** Account/statement fact Import / Refresh is the current next
accuracy milestone, documentation complete and implementation not started in
this task. Broad transactions and the full aggregation layer remain future.
...
**Explicit note:** the current shadow fact milestone does not silently switch
Planning authority and is not authorization for broad transactions or provider
connectivity. The later aggregation platform is not a prerequisite for the
frozen Planning foundation or recovery evidence.
...
- **AutoPay schedule authority and transactional verification (2026-08-03)** — Weekly AutoPay refuses missing, unrecognized, or mismatched Weekday data even when a legacy Due-Day candidate remains visible for manual reconciliation. Occurrence identity is built from stable calendar components, and monthly/expanded AutoPay verifies both the canonical Cash Flow value/format and a newly written immutable marker; an audit failure restores and verifies the exact prior cell.

**Backward compatibility:** blank `Weekday` / `Anchor Date` → legacy Due Day occurrence display and manual handling. Monthly, Biweekly legacy fallback, and other frequencies remain unchanged. Weekly unattended AutoPay deliberately fails closed when Weekday is missing or mismatched; Due Day is never its silent write fallback. Cash Flow and Activity history remain actuals and are not rewritten.

**Runtime validated:** Original Recurrence Engine V2: Laith, Lutfi, M1, Robinhood, San Jose House Cleaning. The 2026-08-03 bounded user check additionally confirmed corrected Laith/Lutfi/M1 Weekly AutoPay timing and canonical negative-currency display on the reviewed source; the two new disposable harness scenarios remain implemented but not yet runtime-executed.
...
High-level mirror; **authoritative copy lives in `TODO.md → Future Feature — Shared Entity Lifecycle Framework`** and numbered delivery scope lives in `FULL_BETA_REMAINING_PLAN.md → 3o`. **Status: active phased rollout. Debt, Bank Accounts, and Investments are complete; Bills is source-complete pending isolated runtime and bounded owner acceptance; Houses and Income Sources remain. Reference implementation: the Debt lifecycle (`Active → Stop Tracking → Inactive → Reactivate`, commit `893d50d`).**
...
Resume Houses → Income Sources after the core Rolling Financial Plan
...
- Automatic external execution (Bills Pay and Quick Add are explicit recording
  paths; Next Actions only routes)
...
- **Bills** (Cash Flow tab) — Three views: **Due this period**, **Add bill**, and **Manage bills**. Manage lists active rows for Edit/Stop tracking and a counted inactive inventory for Reactivate. `reactivateBillFromDashboard` locks and stale-checks the exact preserved row, refuses an active duplicate, restores `Active = Yes`, and records `bill_reactivate`; Add refuses both active and inactive matching identities so it cannot become a recovery path. Runtime validation for this lifecycle candidate remains pending. Edit remains an in-place guarded update and a no-diff save returns **No changes.**
...
- House values — **Update** / **Add new** / **Manage houses**; Manage lists active properties and routes the existing guarded Update / Stop tracking actions. Soft deactivate via `deactivateHouseFromDashboard` in `house_values.js` sets **Active = No** on every **INPUT - House Values** row for the house and on the mirror **SYS - House Assets** row; preserves history, the **HOUSES - {House}** sheet, and name reservation. Logs **`house_deactivate`**. Inactive houses drop out of the House Values dropdown, House Expenses selector, and Property Performance rows/cards — all via the shared `getInactiveHousesSet_` rule: explicit `No / n / false / inactive` = inactive; blank = active.
- House expenses
- Bank accounts — **Update** / **Add new** / **Manage accounts**; Update remains the dated-balance workflow, while Manage owns one-action **Edit** (Account name, Type, Use policy, Priority, Available now, and Minimum buffer), **Stop tracking**, inactive inventory, **Reactivate**, and the secondary Import review / CSV tools. `saveTrackedBankAccountFromDashboard` updates the existing **SYS - Accounts** row and the matching Account Name cell in every **INPUT - Bank Accounts** year block under a Central-safe user lock, with stale/duplicate/ambiguous-history guards and best-effort rollback; month values, formulas, External Account Id, import staging, and immutable Activity history are preserved. Soft deactivate via `deactivateBankAccountFromDashboard` sets **Active = No** across the same history rows and SYS mirror; `reactivateBankAccountFromDashboard` restores those existing rows without recreating the account. Add behavior, formatting inheritance, first-run safeguards, and active filtering remain unchanged.
- Investments — **Update** / **Add new** / **Manage investments**; Manage lists active accounts and routes the existing guarded Update / Stop tracking actions. Soft deactivate via `deactivateInvestmentAccountFromDashboard` in `investments.js` preserves month history, totals / delta, Current Balance, and the reserved name. Existing Add behavior, type choices, activity logging, and active filtering remain unchanged.
...
- **LOG - Activity** — Ledger of script actions (**quick_pay**, bill skip, bill autopay, **bill_add**, **bill_update**, **bill_deactivate**, **bill_reactivate**, **house_expense**, **house_add**, **house_deactivate**, **investment_add**, **investment_deactivate**, **bank_account_add**, **bank_account_deactivate**, **debt_add**, **debt_deactivate**, **donation**, upcoming lifecycle, the Bank Import family — **bank_import_auto_matched** / **bank_import_pending** / **bank_import_ignored_hit** / **bank_import_row_error** (Step 2a ingestion) and **bank_import_review_add_new** / **bank_import_review_match** / **bank_import_review_unlink_match** / **bank_import_review_ignore** / **bank_import_apply_balance** (Step 2b / 2d review + Apply) — and the Planner email family **planner_email_sent** / **planner_email_invalid_recipient** (per-save defers are no longer logged — the count is rolled up onto the next `planner_email_sent` row as `(N saves batched)` via `details.deferredSaveCount`; legacy `planner_email_deferred` rows already on the sheet still render); when **Quick add** runs inside house expense with **`suppressActivityLog`**, the extra **`quick_pay`** row is omitted because **`house_expense`** already logged the save). The Activity table uses a neutral **Action** column. Only eligible `donation` rows show **Remove donation**; Planner/email/import and every other unsupported event show no action control, while the server still rejects forged non-donation deletes. The Beta-critical correction program preserves original audit evidence and adds verified immutable reversals only for explicitly supported financial families (`FULL_BETA_REMAINING_PLAN.md → 5g–5m`). Not a substitute for **OUT - History** (planner snapshots). Created automatically if missing (`activity_log.js`). **Activity** page: **getActivityDashboardData** — filters, sort, up to **500** matches, **20** per page. Bill Stop/Reactivate rows render as non-monetary lifecycle evidence with **—** in Amount.
...
- bank_accounts.js = `addBankAccountFromDashboard`, `saveTrackedBankAccountFromDashboard`, `deactivateBankAccountFromDashboard`, `reactivateBankAccountFromDashboard`, `getBankAccountUiData` (active/inactive management rows + Type and Use Policy options). Edit coordinates the SYS row and every year-block name without changing balance history or import identity. Lifecycle writers self-heal **Active** on every **INPUT - Bank Accounts** year block and **SYS - Accounts**; totals / delta rows remain intact; appended SYS rows retain canonical neighbor formatting. **Priority** remains the canonical 1–99 stored value (Add defaults to 9).
...
- bills.js = `addBillFromDashboard`, `updateTrackedBillFromDashboard` (in-place edit on an active row — Payee / Default Amount / Due Day / Frequency / Payment Source / Category / Autopay / Varies / Notes; per-field diff with **only changed cells** written; refuses inactive rows and stale `expectedPayee`; no-op saves return `No changes.` without logging; never moves the row, never touches Active / Start Month, never retroactively rewrites Cash Flow rows), `deactivateBillFromDashboard`, and `reactivateBillFromDashboard`; case/whitespace-tolerant header lookup; self-heals **INPUT - Bills** by auto-adding missing optional columns (Payment Source, Category, Frequency, Start Month, Notes) before append. Active/inactive management readers expose the preserved row identity; Reactivate locks and stale-checks that exact row, refuses an active duplicate, and restores only `Active`. Add refuses both active and inactive matching payees, so it cannot replace Reactivate.
