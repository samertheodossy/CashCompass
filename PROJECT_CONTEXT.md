...
- **Stage 3 — Beta Readiness** *(current)* — current work is **P0 Project Stabilization** under `ROADMAP.md`. The known P0 ProductDecision inventory and static Central Tier-2 sweep are complete. The Beta Gate still requires recovery adoption-path evidence, remaining unaudited Golden/Canonical scope, Financial Integrity Phase 3, remaining Validator/Test Harness/Release Readiness coverage, runtime regression evidence, and Family Beta polish. The Bills Pay bridge and Investments convergence are committed; their remaining evidence/scope is described above. Goal: **Family Beta Release Candidate — provably correct, recoverable, and visually production-grade.**
