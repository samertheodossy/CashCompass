...
- **Stage:** **Stage 3 — Beta Readiness (current).** **Stage 1 (Core Platform) and Stage 2 (Product Hardening) are complete.** Maturity (estimated): **Family Beta Readiness ~97–98% · External / Public Beta Readiness ~92%**. The remaining pre-beta work is the **Beta Gate**, in agreed priority order — **(1) Golden Workbook Convergence, (2) Financial Integrity Phase 3 (convergence), (3) Validation Agent** (automated release gate), **(4) Recovery completion** (remaining adoption-path validation), **(5) Workbook/UX polish**; not major feature development. Authoritative stage roadmap (Stage 1–6) + Current Engineering Priorities + Beta Gate live in `TODO.md → Product Maturity Stages`; a high-level mirror is in `## Product Maturity Stages (high-level)` below.
...
- **Current focus — Stage 3 Beta Readiness (the Beta Gate, agreed priority order):** (1) **Golden Workbook Convergence** — converge fresh provisioning toward the Golden Workbook (visual source of truth); first audit complete, convergence passes remaining (Cash Flow / Investments / House Values / Retirement); (2) **Financial Integrity — Phase 3 (convergence)** — canonical basis + Planner/Dashboard/Rolling convergence to $0.01 + Asset/Planner/Dashboard audit modules (*the numbers must reconcile*); (3) **Validation Agent** — the automated release gate that protects the reconciled numbers; (4) **Recovery completion — remaining adoption paths** (Auto-Adopt + Ambiguous + Name-only adoption + Orphan on a disposable account, then flags OFF); (5) **Workbook / UX polish**. Remaining recovery slices (Stage 4): 6D.2b Create New Workbook, 6E.2 Admin Set Mapping.
- **Status snapshot (2026-07-02):** Central Architecture ~95%+; Recovery Architecture ~90% implemented; **Recovery destructive/admin paths validated** (only Auto-Adopt / Ambiguous / Name-only adoption / Orphan remaining); **Financial Integrity foundation shipped** (Audit Framework + Debt Audit + shared Active helper + `NOT_INITIALIZED`; Phase 3 convergence remaining); **Golden Workbook Convergence is Stage 3 priority #1** (first audit complete 2026-07-06); **Validation Agent not started**; Family Beta Readiness ~97–98%; External Beta Readiness ~92%.
...
Stable and family-beta capable; provisioning proven across multiple accounts; the recovery destructive/admin paths are validated; core + lifecycle workflows + Bills Due performance shipped. Remaining work is the Beta Gate, not features (agreed priority order):

- Golden Workbook Convergence (converge provisioning toward the visual source of truth; first audit complete)
- Financial Integrity — Phase 3 (convergence)
- Validation Agent (automated release gate that protects the reconciled numbers)
- Recovery completion — remaining adoption-path validation
- Workbook / UX polish + onboarding + additional beta users
...
- **Stage 3 — Beta Readiness** *(current)* — the **Beta Gate**, in agreed priority order: (1) **Golden Workbook Convergence** (P0/P1, L — newly provisioned workbook converges toward the **Golden Workbook** / visual source of truth until visually indistinguishable from production; **first audit complete 2026-07-06** — ten core sheets rated across four design families, see `GOLDEN_WORKBOOK.md` + `WORKBOOK_PARITY_CHECKLIST.md`; a convergence project, not a redesign), (2) **Financial Integrity — Phase 3 (convergence)** (canonical basis + Planner/Dashboard/Rolling convergence to $0.01 + Asset/Planner/Dashboard audit modules — *the numbers must reconcile*), (3) **Validation Agent** (automated release gate that protects the reconciled numbers), (4) **Recovery completion — remaining adoption paths** (Auto-Adopt / Ambiguous / Name-only / Orphan), (5) **Workbook / UX polish** (loading, empty-states, consistency, onboarding) + release/onboarding review. Goal: **Family Beta Release Candidate — provably correct, recoverable, and visually production-grade.**
...
**Current Engineering Priorities (ranked — Version 1 Alignment):** 1) **Golden Workbook Convergence** *(Stage 3 #1 — provisioning converges toward the Golden Workbook, the visual source of truth; first audit complete; convergence, not redesign)* · 2) **Financial Integrity — Phase 3 (convergence)** *(the numbers must reconcile; foundation shipped)* · 3) **Validation Agent** *(automated release gate that protects the reconciled numbers)* · 4) **Recovery completion — remaining adoption paths** · 5) **Workbook / UX polish** (loading, empty-states, consistency, onboarding) · 6) **Shared Lifecycle Framework rollout** (Debt reference → Bank Accounts → Investments → Houses → Income Sources) · 7) **Product differentiators (V1, External Beta): Money Plan Phase 2 + Income Expected/Due Workflow**.

**Beta Gate (release-readiness target):** before broader beta every release must pass **Golden Workbook Convergence** (first audit complete; convergence passes remaining), **Financial Integrity** (foundation shipped; Phase 3 convergence remaining), the **Validation Agent** (not started), **Recovery Validation** (destructive/admin done; adoption paths remaining), and a **runtime regression checklist** before deployment. *(Stage 3 build order: Golden Workbook → FI Phase 3 → Validation Agent → Recovery → UX polish.)*
