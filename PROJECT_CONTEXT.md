...
- **Stage:** **Stage 3 — Beta Readiness (current).** **Stage 1 (Core Platform), Stage 2 (Product Hardening), P0 Project Stabilization, House Financial Accuracy V1, and Financial Integrity Phase 3 are complete**, and **Recurrence Engine V2 (Weekday & Biweekly scheduling) shipped 2026-07-09.** The capability estimate remains **Family Beta Readiness ~97–98% · External / Public Beta Readiness ~92%**. The last formal independent advocate score is **8.3/10** on isolated Central `@180`; it remains official because the isolated `@196` **7.2/10** run was read-only and evidence-limited, not a comparable writer-inclusive rescore. Isolated Central is `@338`; `REG-068` closed one-save Debt Edit/Rename convergence, and `REG-069` closes Bank Account one-save Edit/Rename plus Stop/Reactivate. Populated retry `FR-02e19b9d-9757-4847-afe0-55fd5a03be32` on `@335` passed the exact Bank lifecycle assertion before a later unrelated correction was interrupted by `HTTP 0`; verified cleanup returned the runner to `active: null`. Both authoritative Account Name columns auto-fit to the longest current value after every CashCompass Bank add or rename and retain a small rendering gutter so the final character is not clipped beside populated cells. Pre-publish product consistency `3o` remains current with Investments next; `3p` commonizes dynamic name/payee-column fit, and `5n` reconciles latest-known versus exact-month totals before any basis change. Customer-facing operations and the broad `3j` audit are deferred, not waived. `BETA_10_OUT_OF_10_PLAN.md` defines the ≥95/100 release standard and monetization foundations; `FULL_BETA_REMAINING_PLAN.md` is the authoritative numbered open inventory with estimates, regression homes, and the optimized execution model.
...
**Current implementation priority (2026-08-07):** hold customer-facing release work and close `3o`, the product-consistency/shared-lifecycle wave. Debt and Bank Account convergence are complete through isolated `@338`; Investments, Houses, and Income Sources follow as separate guarded migrations. Follow-up `3p` commonizes dynamic name/payee-column fit, while `5n` audits and reconciles latest-known Dashboard totals with exact-month ledger totals before any calculation-basis change. The broad `3j` accessibility closeout and operations/support/privacy `6a`–`6f` are deferred until this internal product pass is complete, not waived. Bill occurrence correction (`5j`) remains a later focused redesign. This priority change does not reopen the completed `5h`/`5i` foundations.
...
| Current | Pre-publish product consistency and shared lifecycle | Complete `3o` one guarded module at a time; Debt and Bank are complete through isolated `@338`, followed by Investments, Houses, and Income; then close shared fit item `3p` |
