...
- **Startup routing (blank-workbook classification)** — `getStartupRoutingFromDashboard` in `sheet_bootstrap.js` classifies a workbook as blank only when it has no `INPUT -`, `SYS -`, `OUT -`, or `LOG -` sheets via `workbookHasAnyAppSheet_`. Populated workbooks always land on the normal dashboard and `initDashboard()` sets `window.__cashCompassDashboardInited = true`. `loadOnboardingSection()` in `Dashboard_Script_Onboarding.html` uses that flag as the primary Welcome gate: if it's set when **Setup / Review** opens, the status grid renders synchronously — Welcome cannot render for that session regardless of what the per-step probes (`sheetExists` / `status`) report. Only when the flag is unset (e.g. the startup router routed a fresh workbook straight to onboarding) does the payload-based secondary gate decide: any `sheetExists === true` or any `status !== 'missing'` step still routes to the grid; probe failure fails closed to the grid. Both **Back to Dashboard** on Welcome and **Back to Dashboard** on the status grid call `onboardingBackToDashboard()`, which runs `window.initDashboard()` when needed and switches to `overview` — the grid's Back intentionally exits Setup instead of bouncing back to Welcome. Fails closed: if routing probes fail, the app treats the workbook as populated.
