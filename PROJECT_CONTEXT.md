...
**Current implementation priority (2026-08-23):** preserve the frozen Planning foundation and continue **Cash + Credit Card Import / Refresh**. Credit-card correctness boundaries and real Chase QFX/PDF source proof are closed. **Chase Shadow Import V1 Phase A — contract foundation** is complete locally with additive `STATEMENT_BALANCE`, exact component-versus-canonical APR separation, explicit active-account statement association, and protected semantic replay helpers. Phase B is the provider-profile parser implementation; upload UI, customer Preview/Apply, Financial Facts writes, and Planning authority remain unstarted. Existing Part 2A-1 through 2A-5 are an enabling shadow foundation, not production-ready import or Planning authority. Bills lifecycle is complete in `bb20008`; House tracking/recovery is complete in `0bf7610` with isolated `@391`; Income tracking/recovery plus uniform CashCompass confirmation and zero-count recovery behavior are complete in `cdbef99` with permanent local regressions and owner acceptance. House metadata Edit/Rename remains open; Income Edit/Rename remains unclaimed. `3j` accessibility, final `3q` consistency, `5j`–`5m` corrections, operations/privacy, rewards, bill-gap modeling, broader portfolio optimization, and retirement/property optimization remain separate; none reopens frozen Overview/Debt UX.

**Plaid connectivity checkpoint (2026-08-24):** Plaid is the optional primary connected-data layer for credit cards, bank accounts, investments, and mortgages; manual workflows and QFX/PDF evidence remain supported, while property market value requires a separate future AVM provider. Before any connected-provider investigation, implementation, infrastructure modification, deployment, or environment change, read `PLAID_CONNECTIVITY.md`. It documents the Sandbox infrastructure checkpoint, security boundaries, canonical resource inventory, and next P1 milestone; it does not change the frozen Financial Facts, identity, Planning, bounded-workbook, commit, or deployment boundaries.
...
`SYS - Financial Facts`, the original 23 typed fact contracts plus Phase A's
additive 24th `STATEMENT_BALANCE` contract, exact Effective-As-Of vs
...
- House values — **Update** / **Add new** / **Manage houses**; Manage lists active properties and a counted **Show inactive houses** inventory. Soft deactivate via `deactivateHouseFromDashboard` sets **Active = No** on every matching **INPUT - House Values** year-block row and the exact **SYS - House Assets** mirror, preserving history, the **HOUSES - {House}** sheet, linked Debt evidence, and name reservation; it logs **`house_deactivate`**. The runtime-proven `reactivateHouseFromDashboard` accepts the inactive SYS row plus expected exact name, locks, rejects moved/renamed/duplicate/ambiguous/inconsistent identity state, requires the preserved House Values and `HOUSES` history, changes only exact Active cells back to **Yes**, verifies and rolls back on failure, and logs non-monetary **`house_reactivate`** / **Tracking resumed**. Add still reserves active and inactive names across every House store. Isolated `@391` passed 2/2 scenarios and 50/50 assertions with both fixtures TRASHED and runner OFF; House metadata Edit/Rename is separate.
- House expenses
