...
- central_resolver.js = Central App resolver seam (Phase 1, shipped in `b2798a7`). Defines `getUserSpreadsheet_()` as a one-line pass-through to `SpreadsheetApp.getActiveSpreadsheet()`. Single consumer today is `getCashToUse()` in `cash_to_use.js`; the other 134 `SpreadsheetApp.getActiveSpreadsheet()` call sites are intentionally unchanged. Phase 1 is bound-mode only — no `PropertiesService`, no `openById`, no user mapping, no deployment change. Future Central App phases extend the body of this one function. See `CENTRAL_APP_DESIGN.md`, `CENTRAL_APP_FIRST_RESOLVER_SEAM.md`, `CENTRAL_APP_IMPLEMENTATION_PLAN.md`.
...
> **Phase 1 seam landed (`b2798a7`).** The smallest possible first step of the migration shipped: `central_resolver.js` defines `getUserSpreadsheet_()` as a one-line pass-through to `SpreadsheetApp.getActiveSpreadsheet()`, and `getCashToUse()` is the single migrated consumer. Bound-mode behavior is byte-for-byte preserved. The remaining 134 call sites are intentionally untouched and will migrate one at a time in subsequent phases. **No central-mode behavior, no `PropertiesService`, no `openById`, no user mapping, no deployment change exists yet.** See `CENTRAL_APP_FIRST_RESOLVER_SEAM.md` for the design and `CENTRAL_APP_IMPLEMENTATION_PLAN.md → §5 step 1` for status.
