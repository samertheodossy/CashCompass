...
- **Current schedule signal:** **at risk** after elevating the full Activity
  correction program and reconciling the `@196` P1 health/accessibility
  findings. Rebaseline after the durable correction foundation (`5h`) and
  Overview health trust (`3l`) produce measured implementation evidence.
...
|---|---|---|
| Completed through Jul 26 | HTTP 0 recovery, exact-candidate ownership, core Bills Pay, tracked-editor convergence, and Activity action truth | Permanent regressions plus isolated `@196` read-only Activity PASS; prior `@195` writer-inclusive V4/V3 suites remain green |
| Aug 21–27 | Overview health trust; controlled stale/failure, Skip/Stop-tracking safety, Retirement runtime; begin Activity correction foundation and direct Cash Flow correction | Health state reconciles with prerequisites/freshness; critical reliability journeys pass; durable correction identity/preconditions advance |
