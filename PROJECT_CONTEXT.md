...
- **Next milestones (see `TODO.md → Priority-Ranked Roadmap (P0–P4)`):** immediate priority is **Release stabilization** (P0 — final documentation cleanup, Central migration verification, remaining ProductDecision items, beta readiness review); the next **engineering** milestone is **Validator Phase 2** (P1 — provisioning / conditional-format / formula / schema / named-range validation + Workbook Health + release-readiness gate, `VALIDATOR_ARCHITECTURE.md`); the next **product-model** milestone is **House Financial Accuracy** (P2 — rental-property cash-flow accuracy incl. financing costs, one shared house cash-flow model, `TODO.md → House Financial Accuracy`).
