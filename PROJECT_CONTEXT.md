...
- **Status snapshot (2026-07-26):** Central Architecture ~96%+; the major platform, recovery, financial-integrity, workbook, Validator, Harness, and Release Readiness foundations are shipped. Bills completion is closed through `@193`; tracked-editor convergence is closed through `@195`; Activity action truth (`5g` / `REG-020`) is runtime-closed on isolated `@196`, where unsupported events showed no misleading control and the browser had no errors. The prior `@195` First-Run V4 10/10 and Populated V3 13/13 runs remain the latest writer-inclusive evidence with Restricted fixtures and verified Trash cleanup. The `@196` read-only advocate review added `3l` Overview health prerequisite/freshness trust, expanded `3j` programmatic labels/target sizes, and `3m` form readiness/customer language, while mapping the remaining findings to `2d`, `3b`, `3c`, `3h`, and `3i`. Every item has a planned permanent server/UI/browser regression home. Activity correction continues at `5h`–`5m`; the 20-pair Performance campaign remains parked for the exact candidate. Beta remains `@106`; isolated Central validation is `@196`; bounded remains user-controlled and was not used by the advocate or Harness.
