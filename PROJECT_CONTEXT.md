...
- **Next milestones:** Resume P3 finished-feeling UX while the long Performance percentile campaign remains intentionally parked. Before broad Beta, restart the full Performance campaign on the exact candidate, ratify p50/p95 budgets, resume/finalize bounded Release Readiness, and run the 10/10 scorecard. A supervised cohort may validate work during these phases, but broad or monetized beta waits for the exact candidate to pass `BETA_10_OUT_OF_10_PLAN.md`.
