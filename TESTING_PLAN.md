...
3. Click through each top nav page: **Overview, Cash Flow (Quick add, Upcoming, Donations, Bills), Assets (House Values, Bank Accounts, Investments), Properties (House Expenses, Property Performance), Planning (This week, Rolling Debt Payoff, Debt Overview, Retirement, Purchase Simulator), Activity**. Within **This week**, verify Overview, Debt, Cash, Investments, Properties, Forecast, and Data. Each panel must render a calm empty state or a setup-aware CTA — never a red error.
...
6. **Activity → Action column / Donation Remove** (optional, if you have a recent donation log row): confirm Planner/email/import and other unsupported rows show no action button; click **Remove donation** on an eligible **Donation** row; confirm the line disappears from **LOG - Activity** and the matching row is removed from **INPUT - Donation** when the fingerprint still matches (otherwise only the log line goes away—read the status line under the filters).
...
**Manual smoke (Activity / house expense)** — when exercising layer 3: open **Activity**, set **from/to**, **Payee**, **Type**, amount min/max; **Apply** and confirm rows match **LOG - Activity**; confirm **Type** options reflect distinct kinds in the sheet. Sort a column; use **Previous/Next** (20 per page); if you have more than 500 matches, confirm the summary notes truncation. Confirm the final heading is **Action**, eligible Donation rows alone show **Remove donation**, and Planner/email/import/Bill or other unsupported rows show no action button. Exercise one disposable Donation removal if you need to verify log + **INPUT - Donation** reversal, and confirm a forged non-Donation request is rejected server-side. Save a **House Expense** with **Add to Cash Flow** and confirm **one** new ledger row (`house_expense`), not an extra `quick_pay`.
