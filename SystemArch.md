...
- **Backend — Google Apps Script (server-side JavaScript).** One large module set. The important files for this briefing:
  ...
  - Feature files (`bank_accounts.js`, `bank_import.js`, `debounce_planner.js`, `investments.js`, `house_values.js`, `house_expenses.js`, `donations.js`, `upcoming_expenses.js`, `debts.js`, `bills.js`, `property_performance.js`, `retirement.js`, `purchase_simulator.js`). `bank_import.js` owns the full Bank Import pipeline: Step 2a ingestion (`processBankImportBatch_` — ignored-check, exact-id auto-match, pending staging upserts, dedupe) and Step 2b / 2d review + Apply (`getBankImportReviewData`, `addStagedBankAccountAsNew` link-only, `matchStagedBankAccountToExisting` writes only `External Account Id`, `unlinkMatchedStagedBankAccount` Change-match, `ignoreStagedBankAccount` permanent ignore, `applyStagedBankAccountBalance` writes the staged snapshot to the matching month via the Bank → Update path with `bankImportEnsureInputRowForApply_` auto-creating a missing **INPUT - Bank Accounts** row inside an existing Year block when needed); Step 3a CSV-paste wrapper (`processBankImportCsvPasteBatchFromDashboard`, gated by UI flag `BANK_IMPORT_CSV_PASTE_ENABLED`); dev harness gated by `BANK_IMPORT_DEV_TOOLS_ENABLED`. Every entry point is `LockService` wrapped. `debounce_planner.js` owns the planner-email debounce queue: per-save defers bump `LAST_SAVE_AT` + an internal deferred counter in `DocumentProperties`, a 5-minute time-driven `debouncePlannerEmailRun` trigger sends one consolidated email after a 10-minute quiet window, and the deferred count is rolled up onto the `planner_email_sent` activity row as `(N saves batched)` instead of writing one `planner_email_deferred` row per save. `bills.js` owns the **Manage bills** flows (`addBillFromDashboard`, `updateTrackedBillFromDashboard`, `deactivateBillFromDashboard`) and uses case/whitespace-tolerant header lookup with self-heal for missing optional **INPUT - Bills** columns. `updateTrackedBillFromDashboard` is the in-place edit path for active rows (Payee / Default Amount / Due Day / Frequency / Payment Source / Category / Autopay / Varies / Notes); it diffs against the current sheet values, writes only the cells that actually changed, refuses inactive rows + stale `expectedPayee`, never moves the row, never touches Active / Start Month, never retroactively rewrites the Cash Flow row seeded at Add time, and a no-diff save returns `No changes.` without writing or logging. `house_values.js` owns the house lifecycle: `addHouseFromDashboard` + `deactivateHouseFromDashboard` (soft sale — flips **Active = No** on every **INPUT - House Values** row across year blocks and the mirror **SYS - House Assets** row; never deletes or renames the **HOUSES - {House}** sheet); shared `getInactiveHousesSet_` is the single inactive-house rule consumed by the House Values dropdown (`getHouseUiData`), House Expenses selector (`getHouseExpenseUiData`), and Property Performance rows + portfolio totals (`getHouseNamesFromHouseAssets_` in `property_performance.js`).
