...
- **`INPUT - Settings` — 16pt header + 14pt body (2026-07-20).** Settings is an Analytical / Configuration input grid, not a flat `SYS -` backing sheet, so it follows the canonical non-SYS typography: `CANON_FONT_HEADER_` (**16pt**) for the Key / Value header and `CANON_FONT_BODY_` (**14pt**) for editable rows. Applied only when `ensureInputSettingsSheet_()` first creates the sheet; existing populated Settings sheets are never restyled. This resolves the final known P0 typography ProductDecision. A manual Canonical-workbook visual alignment may follow without blocking P0.
