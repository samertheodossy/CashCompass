...
- **`INPUT - Cash Flow <year>` — Summary row financial-health coloring (2026-07-11).** The `Summary | Cash Flow Per Month` net values are a **financial-health cue**: **positive → green**, **negative → red**, **zero → neutral black** (`$0.00`). Implemented through **number format, not conditional formatting** — `[Green]$#,##0.00;[Red]-$#,##0.00;$#,##0.00` — because a number format is simpler and safer, is **inspectable by the current Validator today** (it snapshots number formats but not conditional-format rules — CF validation is a future Validator Phase 2 capability), and extends the negative-red format already on these cells. **Scope: Summary row month values + Total only** — Income/Expense row formatting, formulas, widths, row heights, borders, background, and typography are unchanged. Implemented via `CASH_FLOW_SUMMARY_HEALTH_NUMBER_FORMAT_` in `writeCashFlowSummaryFormulas_` (`cashflow_setup.js`). **Existing workbooks self-heal narrowly** the next time `writeCashFlowSummaryFormulas_` runs (every Quick Add) — allowed because that function already rewrites the Summary formulas/formats and touches only the Summary value cells. See `GOLDEN_WORKBOOK.md → Financial Ledger family`.
