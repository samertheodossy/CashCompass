...
> Current phase: **Stage 3 — Beta Readiness.** The primary architecture is now the **Central App** (a single deployed web app that provisions each allow-listed user their own Drive-owned workbook automatically) — this page covers the **bound / local** first-run path, which still works and is used for bound development + runtime validation. For the Central push/deploy workflow see `PROJECT_CONTEXT.md → Deployment & Push Workflows` and the `CENTRAL_APP_*.md` docs. The V1 trust baseline is complete — a **blank / fresh workbook is stable end-to-end**. The dashboard, Setup / Review, Planning tabs, Assets, Cash Flow, and Activity all render calm empty states instead of `Missing sheet …` crashes, and the planner email only sends when a recipient is explicitly configured in `INPUT - Settings.Email` and the planner summary is meaningful. A known-good template copy is the fastest path to useful numbers, but it is not required to avoid crashes. Newly provisioned workbooks are styled to the **Canonical (Golden) Workbook** standard on first-create (`GOLDEN_WORKBOOK.md`, `ENGINEERING_STANDARDS.md`).
>
