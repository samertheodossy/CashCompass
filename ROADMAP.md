...
- **Current active milestone:** **Reliability and trustworthy evidence (`FULL_BETA_REMAINING_PLAN.md → Group 1`).** HTTP 0 recovery `1a`, dedicated exact-owner proof `1b`, and the Bills product decision (`2a`) are complete through isolated `@193`. Next close the tracked-editor consistency gap for Bank Accounts, Investments, and Houses while controlled reliability journeys `1c`–`1e` remain sequenced for later evidence. `UX-01` and the subsequent contained UX waves are already committed and runtime-validated.
- **Independent advocate gate added 2026-07-23:** the approved isolated interactive writer journey confirmed P1 trust defects in formatted bank-amount replacement, Income/Setup classification consistency, Bills Pay handoff/occurrence feedback, and normal-path implementation terminology. The bank replacement defect is runtime-closed on isolated `@175`, the Income/Setup fix on `@176`/`@178`, the contained language/responsive wave on `@177`, `REG-017` on `@179`, exact-owner `REG-015` on `@181`, readiness-refresh `REG-019` on `@182`, and Bills completion semantics on `@193`. The current advocate score is **8.3/10** pending a fresh full-score run. Retirement/failure-path evidence, medium/narrow replay, and accessibility remain open. No Beta, bounded, mapped-user, Golden, or configured-default workbook was used. These findings are sequenced in `TODO.md → UX Backlog (Version 1) → Independent advocate priorities`.
- **Tracked-editor consistency polish added 2026-07-24:** the Bills slice is complete with **Due this period · Add bill · Manage bills**, Setup preserving Add intent, a direct payment side panel, deterministic occurrence clearing, Cash Flow confirmation, and six-month payment history. The follow-on asset-editor wave will converge Houses, Bank Accounts, and Investments on **Update · Add new · Manage**, matching the established Debt pattern. Bank Import / Review imports remain supported but move under account management as secondary tools rather than competing with everyday actions. Remaining sequence: Bank + Investment Manage views and regressions (2–3 h) → House Manage view and regressions (1–1.5 h) → cross-editor language/focus/empty-state/mobile review (30–45 min). Preserve existing schemas, calculations, writers, Stop tracking safeguards, and Back-to-Setup behavior.
...
**Next active item:** isolated `@181` exact-owner run `RR-0e6941fb-6548-4c45-b5c3-6304ad0af686` passed Workbook Health, all 15/15 disposable server checks, and dedicated Populated Dashboard browser run `FR-13656973-a9c6-49ed-a54a-d6731daf01b6` with 12/12 assertions, exact candidate attribution, Restricted sharing, zero errors, and verified Trash cleanup. This runtime-closed `REG-015` ownership and exposed `REG-019`; isolated `@182` then proved **Refresh status** reconciles that browser evidence as `PASS / Verified` while preserving the exact run as `IN_PROGRESS`. `1b` is complete. `REG-018 / 1a` is also complete on deterministic injected transport coverage plus the clean `@181` deployed integration replay; no unsupported forced-live HTTP 0 claim is made. Beta remains `@106` and bounded was untouched. Next discuss—do not assume—the Bills Pay/Quick add/next-occurrence contract before implementing it, then close controlled failure/stale and Retirement readiness gaps. Keep Performance Planner parked, and never treat a diagnostic browser run as release evidence.
...
- **Tracked-editor navigation consistency — planned immediately after Bills.** Use one recognizable hierarchy for long-lived financial items: Bills **Due · Add · Manage**; Houses / Bank Accounts / Investments / Debts **Update · Add · Manage**. Setup deep-links must preserve whether the customer chose Add or Manage. Manage views provide an active-item inventory and route existing guarded Update / Edit / Stop tracking actions without introducing schema or financial-calculation changes. Bank import workflows become secondary Manage tools. Deliver and validate this in the four small waves recorded under **Now / next**, not as one broad refactor.
