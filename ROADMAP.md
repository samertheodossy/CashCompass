...
- ✅ **Money-format consistency `3n` / `REG-067` on isolated `@331`** — normal-path finite money now uses grouped `$x.xx` with signs before `$`; percentages, counts, ages, dates, numeric-entry behavior, stored values, schemas, and writers are unchanged. Populated V12 run `FR-f7719f23-9248-41fa-9f5c-7327db963ce3` passed 26/26, including the cross-surface money sweep and clean console, with Restricted single-owner sharing and verified cleanup. Beta stayed `@106`; bounded was untouched.

**Now / next:**

- **Current milestone:** **`3j`, the separately scoped responsive/accessibility closeout.** Money-format consistency `3n` is closed on isolated `@331`. Beta remains `@106`; bounded deployment and workbook control remain with the user.
...
| Current | Run `3j` as its own responsive/accessibility task. |
| Next | Return to deferred compound Activity correction `5j`–`5m`. |
...
**Next active item:** `3j`, the separate responsive/accessibility closeout.
Money-format consistency `3n` is runtime-complete on isolated Central `@331`,
and combined UX items `3e`–`3i`/`3m` remain complete on `@325`. Bill occurrence
correction (`5j`) is deferred for the following focused redesign. The shared
...
**Remaining broad-Beta sequence:** close the separate `3j` task; return to the
