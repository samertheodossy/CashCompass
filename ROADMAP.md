...
- **Current engineering milestone:** **Complete automated release evidence** (P1): Release Readiness console controls, completion of the paused performance campaign, and the final bounded verdict.
...
5. **Release Readiness gate** — bounded orchestration and privacy-safe archived evidence are source-ready, and the inventory now consumes all three saved browser-suite PASS records. The verdict remains NOT READY until performance-budget ratification, console controls, and the final bounded evidence run close. The 20-pair Performance Planner campaign is resumable and intentionally paused after six confirmed pairs; no p50/p95 verdict is claimed from the partial sample.
...
**Next active item while sampling is deferred:** implement the **Release Readiness console controls**. Later, resume the remaining Performance Planner pairs, make the performance-budget ProductDecision, and run the final bounded evidence gate.
