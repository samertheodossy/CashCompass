...
4. **Scenario packs** *(SMOKE → REGRESSION → RECOVERY → STRESS; full E2E roadmap + coverage matrices + build order in `REGRESSION_SUITE_PLAN.md`; historical-bug registry in `REGRESSION_SCENARIOS.md`)* — Bills Regression, Recovery Regression, Quick Add Reliability, Representative Populated Fixture, and the cross-cutting **Central Safety Regression** suite are runnable. Next preserve the recent manual sessions as `SUITE-FIRST-RUN-UX-E2E`, `SUITE-POPULATED-DASHBOARD-E2E`, `SUITE-RECOVERY-LIVE`, `SUITE-PERFORMANCE-PLANNER`, and `SUITE-BILLS-PAY-E2E`. UI runner and explicit-ss planner seams gate those deeper packs; Release Certification must execute suites in bounded chunks rather than one timeout-prone mega-suite.
5. **Release Readiness gate** *(aggregate Harness + Validator results into a pre-beta go/no-go — `RELEASE_READINESS.md`)*
6. **Validation & Testing admin console** — **◑ V1 done.** *(operator UI surface — `VALIDATION_TESTING_CONSOLE.md`; route `?view=validation`. Live: Target + Workbook Health (Provisioning / Drift / Schema Evolution) + dynamic one-scenario or one-suite Test Harness execution with detailed reports. Future: multi-select pack composition, Formula/CF buttons, bounded Release Certification orchestration, and Release Readiness verdict.)*
