...
1. **Validator Phase 2A/2B — Provisioning + Drift + Schema Evolution** *(the judge; `validator_health.js` — `VALIDATOR_ARCHITECTURE.md → §10.12`)*. Phase 2A (provisioning runner) and 2B′ (advisory Workbook Drift runner) are implemented. Phase 2 distinguishes **three questions**: **Provisioning** = *"created correctly?"* (required sheets/headers; **gating/FAIL** — `§10.0a`); **Workbook Drift** = *"diverged cosmetically?"* (widths, row heights, styling, formulas, CF; **advisory** — `§10.0a`); **Schema Evolution** = *"supported legacy schema?"* (version-attributable deltas — missing `SYS - Meta`, header ordering, frozen-pane conventions — reconciled to a **Workbook Type** + **Compatibility** verdict; **advisory, version-aware** — `§10.0b`). Next slice **2B″ — Schema Evolution / version compatibility** (design done, `§10.0b`; not implemented): move version-attributable structural checks out of the provisioning gate into an advisory, era-aware lens so legacy/bound workbooks read as *Compatible Legacy* rather than provisioning failures.
