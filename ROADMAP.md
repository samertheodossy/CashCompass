...
- **Current active milestone:** **Reliability and trustworthy evidence (`FULL_BETA_REMAINING_PLAN.md → Group 1`).** HTTP 0 recovery `1a`, dedicated exact-owner proof `1b`, and the Bills product decision (`2a`) are complete through isolated `@193`. The tracked-editor consistency base wave passed isolated Central `@194`; the follow-up Save-only Update / destructive-actions-in-Manage cleanup passed First-Run V4 10/10 and Populated V3 13/13 on isolated `@195`. Controlled reliability journeys `1c`–`1e` remain sequenced afterward. `UX-01` and the earlier contained UX waves are already committed and runtime-validated.
...
- **Tracked-editor consistency polish completed 2026-07-24:** the Bills slice is complete with **Due this period · Add bill · Manage bills**. Houses, Bank Accounts, and Investments now use the consistent **Update · Add new · Manage** hierarchy, explicit Setup Add/Manage handoffs, responsive active-item inventories, and a shared UI layer for mode selection, list rendering, focus, and empty states. Existing feature-specific writers and Stop tracking safeguards remain separate. Bank Import / Review imports remain supported as secondary tools under Manage accounts. First-Run V3 run `FR-9b0914ad-1125-46c2-afe2-4f82688f1646` passed 10/10 and Populated V2 run `FR-465d30b2-77da-46db-bc67-4070394ee1c7` passed 13/13 on isolated Central `@194`, each with Restricted single-owner sharing, zero browser errors, and verified Trash cleanup. A user-approved follow-up removes Stop tracking from all asset/debt Update panels (and the legacy sidebar) so Update stays Save-only and Manage is the single lifecycle-maintenance surface; V4/V3 browser evidence is required before that refinement is runtime-closed.
