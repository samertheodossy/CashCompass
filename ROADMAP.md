...
- **Current milestone:** **`3o`, the pre-publish product-consistency and shared-lifecycle closeout.** Debt Edit/Rename convergence is complete on isolated `@334`; Bank Accounts, Investments, Houses, and Income Sources remain sequenced migrations because their names and lifecycle state are referenced across history blocks, system indexes, linked debts, or Cash Flow rows. Customer-facing operations/support/privacy work (`6a`–`6f`) and the broad `3j` responsive/accessibility closeout are deferred until this product pass is complete, not waived. Beta remains `@106`; bounded deployment and workbook control remain with the user.
...
| Current | Close `3o` product consistency and shared lifecycle one guarded module at a time: Bank → Investments → Houses → Income. |
...
lifecycle closeout. Debt Edit/Rename convergence is runtime-complete on isolated
Central `@334`; migrate Bank Accounts, Investments, Houses, and Income Sources
