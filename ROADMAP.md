...
- **Current milestone:** **`4g`, Retirement scenario-load investigation.** Measure why opening Retirement waits for three scenario paths, then optimize only if the selected scenario can render sooner without weakening calculation truth. Follow with `3n`, a customer-visible money-format consistency audit, then run the separately scoped `3j` responsive/accessibility closeout. The combined `3e`–`3i`/`3m` UX wave is committed in `993720c`; its `REG-065` loading-truth follow-up is runtime-complete on isolated `@326`. Bill occurrence correction (`5j`) remains explicitly deferred for its later focused redesign. Beta remains `@106`; bounded deployment and workbook control remain with the user.
...
- **Remaining performance work:** `4a`–`4f` are complete for their current evidence candidates. `4g` is a newly observed Retirement-tab latency investigation and is separate from the ratified Financial Plan refresh and closed ordinary-Save work. Any source or deployment change before the frozen-candidate `8b` gate requires a fresh Performance replay because exact-candidate evidence cannot be inherited.
...
| Current | Investigate and, if justified, improve selected-scenario Retirement loading (`4g`), then audit customer-visible money formatting (`3n`). |
...
**Next active item:** `4g`, Retirement scenario-load latency, followed by `3n`,
the money-format audit. Combined UX items `3e`–`3i` and `3m` are runtime-complete
on isolated Central `@325`; `3j` remains a separate responsive/accessibility
task. Bill occurrence correction (`5j`) is deferred for a later focused
redesign. The shared `5h` envelope and direct Quick Add/Donation correction `5i`
remain committed foundations and are not reopened by this priority change.

**Remaining broad-Beta sequence:** close `4g`, `3n`, and the separate `3j` task;
return to the deferred `5j`–`5m` correction families; finish `5a`–`5f`
