...
**Next active item:** checkpoint the user-reviewed, display-only `UX-01` Overview hierarchy and balanced grid. The Bank formatted-money replacement is runtime-verified on isolated `@175`; local fixes and regression guards are now present for exact-candidate browser evidence (`REG-015`) and Income/Setup truth (`REG-016`). Runtime-replay those two fixes on the isolated candidate, then continue the Bills Pay handoff and remaining advocate P1 issues one at a time before ordinary `UX-02` polish. Keep Performance Planner as an explicit parked gate, then restart its full exact-candidate percentile campaign, ratify the budget ProductDecision, and produce the final bounded verdict before broad Beta release.
