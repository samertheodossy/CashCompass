...
3. **Disposable populated-fixture hardening** — create populated fixtures through Central so they remain accessible under the intentional `drive.file` boundary; seed representative Bank / Investment / House / Debt data automatically; default every fixture to **Restricted** sharing and fail closed when a fixture is link-accessible; verify cleanup returns the fixture to Trash without touching owner/bounded workbooks.
...
6. **Validation & Testing admin console** — **◑ V1 done.** *(operator UI surface — `VALIDATION_TESTING_CONSOLE.md`; route `?view=validation`. Live: Target + Workbook Health (Provisioning / Drift / Schema Evolution) + a single-scenario Test Harness card. Future: multi-suite selector, Formula/CF buttons, Release Readiness verdict.)*
