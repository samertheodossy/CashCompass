...
- **Remaining performance work:** `4a`–`4e` are complete. Planner budget ratification under `4f` is complete for `8aa4bf5` / isolated `@318`, and `4d` cross-flow measurement is complete on `@319`. Ordinary Save completion measured 8.332 s against the directional 6 s target, so investigation/optimization and remeasurement remain under `4f`. Any source or deployment change before the frozen-candidate `8b` gate requires a fresh Performance replay because exact-candidate evidence cannot be inherited.
...
| Candidate preparation | Close ordinary Save performance follow-up under `4f`, financial/workbook/recovery proof `5a`–`5f`, operations/support `6a`–`6f`, and monetization-ready architecture `7a`–`7f`. |
...
2. `4f` — investigate/optimize ordinary Save completion and remeasure it; `4d` measurement is complete;
