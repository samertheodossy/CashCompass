...
- **Current milestone:** **`3n`, customer-visible money-format consistency**, followed by the separately scoped `3j` closeout. Retirement performance `4g` is closed on isolated `@328`. Beta remains `@106`; bounded deployment and workbook control remain with the user.
...
| Current | Audit customer-visible money formatting (`3n`), then complete the separate `3j` accessibility task. |
...
**Next active item:** `3n`, customer-visible money-format consistency, followed by
the separate `3j` responsive/accessibility closeout. Combined UX items `3e`–`3i` and `3m` are runtime-complete
...
**Remaining broad-Beta sequence:** close `3n` and the separate `3j` task;
