...
- **Remaining performance work:** `4a`–`4f` are complete for their current evidence candidates. Planner budgets are ratified for `8aa4bf5` / isolated `@318`; `4d` cross-flow measurement is complete on `@319`; and ordinary Save investigation/optimization is closed on `@322` with a documented 6.520/9.418 s p50/p95 near-miss against the strict 6 s completion budget. Any source or deployment change before the frozen-candidate `8b` gate requires a fresh Performance replay because exact-candidate evidence cannot be inherited.
