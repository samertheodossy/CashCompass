...
| 4.9.2 | Add a House. | `INPUT - House Values` and `SYS - House Assets` created (if not already). | | `INPUT - House Values`, `SYS - House Assets` (if not already) | | (one `house_add`) | | |
| 4.9.3 | Edit the House Value for the current month. | Write succeeds. | | none | | (one `house_value_update`) | | |
...
| 3 | (Fill in any new gap surfaced by the runtime test.) | | | |
