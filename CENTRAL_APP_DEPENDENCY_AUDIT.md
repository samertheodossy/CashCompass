...
> **Current Tier-2 closeout sweep (2026-07-20).** A fresh literal scan of all Apps Script and HTML sources found no direct `SpreadsheetApp.getActiveSpreadsheet()` call in a Central user-facing production path outside the resolver. The executable residuals are intentionally classified: `central_resolver.js` is the bound-mode fallback inside the canonical seam; `home.js` is the bound-workbook **Rebuild HOME** menu utility; `sortSheets.js` is a manual bound-workbook utility; `onboarding.js → ensureOnboardingTestSheetsFromDashboard()` is developer Test Setup infrastructure that deliberately clones within the active bound workbook; and `test_harness_core.js` reads the active workbook only as a fail-closed guard that refuses destructive work against it. Other literal matches are comments documenting the migration. The user-facing Tier-2 resolver sweep is therefore **closed by source inspection**; these bound/dev/safety residuals are explicitly retained and are not Central migration gaps. Runtime Recovery Validation 6F remains a separate P0 evidence gate.

> **Update — Phases 1–4 shipped (`b2798a7` → `1b68c71` → `72d82b1` → `99bcf37`).** Of the ~135 production call sites of `SpreadsheetApp.getActiveSpreadsheet()` catalogued below, **four** have been migrated through the resolver: `getCashToUse()` in `cash_to_use.js:77` (Phase 1, `b2798a7`), `getQuickAddPaymentUiData()` in `quick_add_payment.js:35` (Phase 2, `1b68c71`), `getDebtPayoffReadData()` in `debt_payoff_projection.js:17` (Phase 3, `72d82b1`), and `getDebtPaymentBreakdownForDashboard()` in `dashboard_data.js:1235` (Phase 4, `99bcf37` — **first dashboard resolver seam**). All four call `getUserSpreadsheet_()` defined in `central_resolver.js` as a one-line pass-through (body unchanged since Phase 1). The remaining **131** call sites are intentionally unchanged. The §3 inventory below is preserved as the original audit; updated counts after Phase 4 are: `cash_to_use.js` now has 0 (was 1), `quick_add_payment.js` now has 2 (was 3 — lines 185 and 248 remain on the platform call as documented write-path entries), `debt_payoff_projection.js` now has 0 (was 1 — **first fully resolver-routed module**, achieved Phase 3), `dashboard_data.js` now has 9 (was 10 — first dashboard seam shipped; 9 residual sites remain across the snapshot wrapper, four history readers, two ensure-backed reads, the planner-email helper, and the `skipDashboardBill` write entry — all intentionally deferred per `CENTRAL_APP_DASHBOARD_SEAM_ANALYSIS.md → §6`), and `central_resolver.js` contains the canonical 1 occurrence inside the resolver body. No other counts change. **Resolver body still the one-line pass-through.** No central mode, no `PropertiesService`, no `openById`, no identity helper, no user mapping, no deployment change in any of the four phases. See `CENTRAL_APP_FIRST_RESOLVER_SEAM.md` / `CENTRAL_APP_SECOND_RESOLVER_SEAM.md` / `CENTRAL_APP_THIRD_RESOLVER_SEAM.md` / `CENTRAL_APP_DASHBOARD_SEAM_ANALYSIS.md` for the per-phase designs and `CENTRAL_APP_IMPLEMENTATION_PLAN.md → §5 step 1` for status.
