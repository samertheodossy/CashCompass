...
| Isolated Central validation | https://script.google.com/macros/s/AKfycbzMaD3Ur0H3VmatL4W2vVHlYhFOXF4cZSSjIcn3SwggbTCs9Q9F1_PH74F16lAFkUlWZQ/exec | Immutable disposable runtime-validation deployment remains `@392`. The exact later presentation source on Central `@HEAD` passed the full local regression suite and editor-only visual verification of both the ordered **Bank accounts → Debts → Investments → Houses** navigation and the compact top-right House inactive control; the owner also visually accepted the presentation. A new immutable version was not created because the Apps Script project reached Google's 200-version limit; no historical version was deleted. House Financial Accuracy remains runtime-proven on `@391`: 2/2 scenarios and 50/50 assertions, including House Stop/Reactivate 23/23; Restricted Provisioning and Drift passed, both marker-verified fixtures were verified TRASHED, and the runner returned OFF. The prior `@387` Capital Allocation and Bills evidence remains accepted; its Bills Edit Integrity fixture had one advisory Drift result while functional and Provisioning gates passed. Central Beta remains version 106. |
