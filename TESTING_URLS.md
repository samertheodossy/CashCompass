...
| Isolated Central validation | https://script.google.com/macros/s/AKfycbzMaD3Ur0H3VmatL4W2vVHlYhFOXF4cZSSjIcn3SwggbTCs9Q9F1_PH74F16lAFkUlWZQ/exec | Immutable isolated deployment is `@403`. Plaid Trial read-only candidate: append `?build=403`, then use **Assets & Liabilities → Bank Accounts / Debts / Investments → Connected**. The standalone Connected Accounts destination is intentionally removed. `@403` preserves an intentionally selected Connected mode while asynchronous manual-account reads finish, in addition to the Bank Accounts panel-structure correction from `@402`; the explicit Trial/real-data warning and no Apply/Save/Accept boundary remain. Beta remains `@106`; bounded is untouched. Earlier House Financial Accuracy evidence remains runtime-proven on `@391`, and prior Capital Allocation/Bills evidence remains accepted on `@387`. |
